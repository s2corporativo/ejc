...
Por padrão este utilitário apenas decifra em diretório temporário, valida o dump
com ``pg_restore --list`` e inspeciona o ``tar.gz`` contra path traversal,
links e tipos especiais. Os arquivos em claro são removidos ao final.

A restauração real é opt-in com ``--restore-test`` e exige
``RESTORE_TEST_DATABASE_URL`` apontando para banco explicitamente descartável.
A chave nunca é aceita na linha de comando para não aparecer em ``ps``.
"""
...
DISPOSABLE_DB_RE = re.compile(
    r"(?:^|[_-])(?:test|teste|homolog|homologacao|restore|restauracao|dr)(?:[_-]|$)",
    re.IGNORECASE,
)
...
class DatabaseTarget:
    ...
    def identity(self) -> tuple[str, int, str, str]:
        return (self.host.lower(), self.port, self.user, self.database)
...
def inspect_pg_dump(path: Path, timeout: int) -> int:
    ...
    if object_count <= 0:
        raise RestoreValidationError("Dump PostgreSQL vazio ou sem objetos restauráveis")
    return object_count
...
def inspect_uploads_tar(path: Path) -> tuple[int, int]:
    ...
    try:
        ...
        raise RestoreValidationError("Arquivo de uploads inválido ou corrompido") from exc
...
def parse_database_url(raw_url: str) -> DatabaseTarget:
    ...
    if not parsed.scheme.startswith("postgresql"):
        raise RestoreValidationError("RESTORE_TEST_DATABASE_URL deve ser PostgreSQL")
...
def restore_database(path: Path, target: DatabaseTarget, timeout: int) -> int:
    ...
    if count_result.returncode != 0:
        raise RestoreValidationError("Banco restaurado, mas a consulta de sanidade falhou")
    try:
        table_count = int(count_result.stdout.strip())
    except ValueError as exc:
        raise RestoreValidationError("Contagem de tabelas restauradas inválida") from exc
    if table_count <= 0:
        raise RestoreValidationError("Restauração concluída sem tabelas no schema public")
    return table_count
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--key-file",
        type=Path,
        help="arquivo somente leitura contendo a chave Fernet; a chave não vai no argv",
    )
    parser.add_argument(
        "--restore-test",
        action="store_true",
        help="restaura em RESTORE_TEST_DATABASE_URL, obrigatoriamente descartável",
    )
    parser.add_argument(
        "--uploads-output-dir",
        type=Path,
...
def run(args: argparse.Namespace) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="ejc_restore_verify_") as temp_dir:
        ...
        if args.uploads_artifact is not None:
            ...
            uploads_size = decrypt_artifact(args.uploads_artifact, uploads_clear, key)
        ...
        if args.restore_test:
            ...
            if uploads_clear is not None:
                if args.uploads_output_dir is None:
                    raise RestoreValidationError(
                        "Backup contém uploads; informe --uploads-output-dir no restore-test"
                    )
                restored_files = extract_uploads(uploads_clear, args.uploads_output_dir)
