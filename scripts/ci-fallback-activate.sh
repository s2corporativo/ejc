...
canon() { if command -v realpath >/dev/null 2>&1; then realpath -m "$1"; else printf '%s\n' "$1"; fi; }
...
validate_scheduler_value() {
  ...
  case "$value" in *$'\n'*|*$'\r'*|*"'"*|*'"'*|*'\'*|*%*|*[[:space:]]*) fail "$label contém caractere inseguro para scheduler";; esac
}
for pair in "ROOT:$ROOT" "LOG_DIR:$LOG_DIR" "REPO:$REPO" "APP_KEY_FILE:$APP_KEY_FILE" "WATCHER_PATH:$WATCHER_PATH" "WORKER_USER:$WORKER_USER" "WORKER_ROOT:$WORKER_ROOT"; do validate_scheduler_value "${pair%%:*}" "${pair#*:}"; done
...
remove_watcher() {
  ...
  if command -v systemctl >/dev/null 2>&1; then
    if [ -f "$UNIT" ] || systemctl --user is-active --quiet ejc-ci-fallback.service 2>/dev/null || systemctl --user is-enabled --quiet ejc-ci-fallback.service 2>/dev/null; then systemctl --user disable --now ejc-ci-fallback.service >/dev/null 2>&1 || rc=1; fi
    if [ -e "$UNIT" ]; then rm -f "$UNIT" || rc=1; systemctl --user daemon-reload >/dev/null 2>&1 || rc=1; fi
  elif [ -e "$UNIT" ]; then rc=1; fi
  remove_cron || rc=1
  return "$rc"
}
scheduler_present() {
...
restore_hooks_path() {
  ...
  local previous; previous="$(cat "$HOOKS_BACKUP")"
  if [ "$previous" = __UNSET__ ]; then git config --local --unset-all core.hooksPath >/dev/null 2>&1 || true; else git config --local core.hooksPath "$previous"; fi
  rm -f "$HOOKS_BACKUP"
}
backup_hooks_path() {
  [ ! -e "$HOOKS_BACKUP" ] || fail "backup residual de core.hooksPath existe"
  if git config --local --get core.hooksPath >/dev/null 2>&1; then git config --local --get core.hooksPath > "$HOOKS_BACKUP"; else printf '%s\n' __UNSET__ > "$HOOKS_BACKUP"; fi
  chmod 0600 "$HOOKS_BACKUP"
}
create_drain() {
  local tmp="$DRAIN_FILE.tmp.$$"
  printf 'draining %s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" > "$tmp"; chmod 0600 "$tmp"; mv "$tmp" "$DRAIN_FILE"
}
...
write_active_state() {
  ...
  chmod 0600 "$tmp"; mv "$tmp" "$ACTIVE_FILE"
}
systemd_user_persistent() {
  command -v systemctl >/dev/null 2>&1 || return 1; command -v loginctl >/dev/null 2>&1 || return 1; systemctl --user show-environment >/dev/null 2>&1 || return 1
  [ "$(loginctl show-user "$(id -un)" -p Linger --value 2>/dev/null || true)" = yes ]
}
cron_persistent() {
  command -v crontab >/dev/null 2>&1 || return 1
  if command -v systemctl >/dev/null 2>&1; then systemctl is-active --quiet cron 2>/dev/null && return 0; systemctl is-active --quiet crond 2>/dev/null && return 0; fi
...
UNIT
    chmod 0600 "$UNIT"; systemctl --user daemon-reload; systemctl --user enable --now ejc-ci-fallback.service; systemctl --user is-active --quiet ejc-ci-fallback.service
  else
    local line
...
if [ "$MODE" = --status ]; then
  ...
  else printf '[fallback-activate] state=inactive draining=%s\n' "$([ -e "$DRAIN_FILE" ] && echo yes || echo no)"; fi
  exit 0
fi
...
if [ "$MODE" = --disable ]; then
  ...
  if ! EJC_REPO="$REPO" EJC_BRANCH_PROTECTION_BACKUP="$PROTECTION_BACKUP" bash scripts/governanca/branch-protection.sh --restore; then rm -f "$DRAIN_FILE"; fail "restauração da proteção falhou; watcher preservado e reativado"; fi
  if ! remove_watcher; then fail "proteção restaurada, mas scheduler não pôde ser removido; drain mantido"; fi
...
if command -v docker >/dev/null 2>&1 && docker info >/dev/null 2>&1 && [ -n "$(docker ps -q 2>/dev/null)" ]; then fail "host possui containers em execução; worker de PR exige host CI dedicado sem serviços co-residentes"; fi
...
for script in scripts/ci-local.sh scripts/ci-fallback.sh scripts/ci-fallback-watch.sh scripts/ci-fallback-activate.sh scripts/ci-worker-isolation.sh scripts/github-app-auth.sh scripts/governanca/ci-local-governanca.sh scripts/governanca/branch-protection.sh; do bash -n "$script" || fail "sintaxe inválida: $script"; done
python3 -m py_compile scripts/ci_evidence.py || fail "ci_evidence.py inválido"
EJC_CI_WORKER_USER="$WORKER_USER" EJC_CI_WORKER_ROOT="$WORKER_ROOT" EJC_FALLBACK_APP_PRIVATE_KEY_FILE="$APP_KEY_FILE" EJC_CI_STATE_ROOT="$CACHE_ROOT" bash scripts/ci-worker-isolation.sh preflight || fail "worker dedicado não atende isolamento"
ok "worker dedicado validado"
if ! EJC_ALLOW_PYTHON_MISMATCH=0 bash scripts/ci-local.sh fast >"$LOG_DIR/activation-preflight.log" 2>&1; then tail -n 100 "$LOG_DIR/activation-preflight.log" >&2 || true; fail "preflight trusted da main falhou"; fi
...
backup_hooks_path
create_drain
HOOKS_CHANGED=0; WATCHER_INSTALLED=0; PROTECTION_CHANGED=0
rollback_enable() {
  local rc=$?; trap - EXIT
  if [ "$rc" -ne 0 ]; then
    if [ "$PROTECTION_CHANGED" -eq 1 ] && [ -s "$PROTECTION_BACKUP" ]; then EJC_REPO="$REPO" EJC_BRANCH_PROTECTION_BACKUP="$PROTECTION_BACKUP" bash scripts/governanca/branch-protection.sh --restore >/dev/null 2>&1 || true; fi
    ...
    [ "$PROTECTION_CHANGED" -eq 0 ] || rm -f "$PROTECTION_BACKUP"
  fi
  exit "$rc"
}
trap rollback_enable EXIT
install_watcher "$SCHEDULER"; WATCHER_INSTALLED=1
scheduler_present || fail "watcher não permaneceu ativo em drain"
git config --local core.hooksPath .githooks; HOOKS_CHANGED=1
