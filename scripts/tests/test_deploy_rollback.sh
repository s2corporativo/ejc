...
grep -Fq 'unlock_env_if_needed || return 1' "$ROOT/scripts/deploy_vps_safe.sh" || fail "restore_env não remove +i antes da restauração"
grep -Fq 'relock_env_if_needed || rc=1' "$ROOT/scripts/deploy_vps_safe.sh" || fail "restore_env não reaplica +i após a restauração"
