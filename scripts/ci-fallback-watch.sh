...
maybe_maintain_state() {
  ...
  if python3 "$ROOT/scripts/ci_evidence.py" prune --root "$EVIDENCE_ROOT" --max-shas "$EVIDENCE_MAX_SHAS" --max-age-days "$EVIDENCE_MAX_AGE_DAYS" --attempts-per-sha "$EVIDENCE_ATTEMPTS_PER_SHA" >"$STATE_ROOT/maintenance-last.json" 2>"$STATE_ROOT/maintenance-last.err"; then
    find "$STATE_ROOT" -maxdepth 1 -type f \( -name '*.result' -o -name '*.infra-retry.json' -o -name 'pr-*.json' -o -name '*.invocation.log' \) -mtime "+$EVIDENCE_MAX_AGE_DAYS" -delete 2>/dev/null || true
...
failure_is_infrastructure() {
  ...
  if [ -n "$stage_log" ] && grep -Eiq "$INFRA_ERROR_RE" -- "$stage_log"; then return 0; fi
...
retry_delay_seconds() {
  ...
  while [ "$i" -lt "$attempts" ]; do
    delay=$((delay * 2))
    if [ "$delay" -ge "$INFRA_RETRY_MAX_SECONDS" ]; then delay="$INFRA_RETRY_MAX_SECONDS"; break; fi
    i=$((i + 1))
  done
  printf '%s\n' "$delay"
}
read_retry_state() {
  local file="$1"
  RETRY_ATTEMPTS=0; RETRY_LAST_EPOCH=0
  [ -s "$file" ] || return 0
...
run_cycle() {
  if [ -e "$DRAIN_FILE" ]; then echo "[fallback-watch] drain ativo; ciclo pulado sem acessar PRs." >&2; return 0; fi
  maybe_maintain_state
  if ! gh auth status >/dev/null 2>&1; then echo "[fallback-watch] gh indisponível/não autenticado neste ciclo; nova tentativa depois." >&2; return 0; fi
  if ! git fetch --quiet origin main; then echo "[fallback-watch] GitHub/fetch indisponível neste ciclo; mantendo estado local." >&2; return 0; fi
  if [ "$(git branch --show-current)" != "main" ] || [ -n "$(git status --porcelain)" ]; then echo "[fallback-watch] checkout deixou de ser main limpa; ciclo recusado." >&2; return 0; fi
  if ! git merge --ff-only origin/main >/dev/null 2>&1; then echo "[fallback-watch] main local não pôde avançar por fast-forward; ciclo recusado." >&2; return 0; fi
  ...
  if [ -z "$prs_json" ]; then echo "[fallback-watch] API de PR indisponível neste ciclo." >&2; return 0; fi
  ...
  while IFS=$'\t' read -r pr sha merge_state updated_at; do
    ...
    marker="$STATE_ROOT/$sha.result"; retry_file="$STATE_ROOT/$sha.infra-retry.json"; pr_state="$STATE_ROOT/pr-$pr.json"; invocation_log="$STATE_ROOT/$sha.invocation.log"
    now="$(date +%s)"; fingerprint="$sha|$updated_at|$merge_state|$AUTO_MERGE"

    if local_evidence_green "$sha"; then
      atomic_text "$marker" success; rm -f "$retry_file"
      if ! pr_state_due "$pr_state" "$fingerprint" "$now"; then continue; fi
      : > "$invocation_log"; chmod 600 "$invocation_log" 2>/dev/null || true
      set +e
      if [ "$AUTO_MERGE" = "1" ]; then bash "$ROOT/scripts/ci-fallback.sh" --pr "$pr" --merge-only >>"$invocation_log" 2>&1; else bash "$ROOT/scripts/ci-fallback.sh" --pr "$pr" --promote-only >>"$invocation_log" 2>&1; fi
      rc=$?; set -e
      [ "$rc" -eq 75 ] && continue
    ...
    if [ -f "$marker" ] && grep -qx failure "$marker"; then
      if [ ! -s "$retry_file" ]; then
        ...
        [ "$RETRY_ATTEMPTS" -lt "$INFRA_MAX_RETRIES" ] || continue
        delay="$(retry_delay_seconds "$RETRY_ATTEMPTS")"
        [ $((now - RETRY_LAST_EPOCH)) -ge "$delay" ] || continue
      fi
    fi

    : > "$invocation_log"; chmod 600 "$invocation_log" 2>/dev/null || true
    started="$now"; set +e
    if [ "$AUTO_MERGE" = "1" ]; then bash "$ROOT/scripts/ci-fallback.sh" --pr "$pr" --merge >>"$invocation_log" 2>&1; else bash "$ROOT/scripts/ci-fallback.sh" --pr "$pr" >>"$invocation_log" 2>&1; fi
    rc=$?; set -e
    if [ "$rc" -eq 75 ]; then atomic_text "$marker" pending; continue; fi
    if local_evidence_green "$sha"; then atomic_text "$marker" success; rm -f "$retry_file"; write_pr_state "$pr_state" "$pr" "$sha" "$updated_at" "$merge_state" "$(date +%s)" suite-success; continue; fi
    if [ "$rc" -eq 0 ]; then atomic_text "$marker" pending; continue; fi

    if failure_is_infrastructure "$sha" "$started" "$invocation_log"; then
      read_retry_state "$retry_file"; attempts=$((RETRY_ATTEMPTS + 1)); now="$(date +%s)"; write_retry_state "$retry_file" "$sha" "$attempts" "$now"; atomic_text "$marker" failure
    else
      rm -f "$retry_file"; atomic_text "$marker" failure
