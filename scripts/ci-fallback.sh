...
post_full_check() {
  ...
  if [ -n "$FULL_CHECK_ID" ] && [ "$state" != "pending" ]; then
    result="$(printf '%s' "$payload" | ejc_github_app_gh_api -X PATCH "repos/$REPO/check-runs/$FULL_CHECK_ID" -H 'Accept: application/vnd.github+json' --input - 2>/dev/null)" || { STATUS_SYNC_PENDING=1; return 75; }
  else
    result="$(printf '%s' "$payload" | ejc_github_app_gh_api -X POST "repos/$REPO/check-runs" -H 'Accept: application/vnd.github+json' --input - 2>/dev/null)" || { STATUS_SYNC_PENDING=1; return 75; }
  fi
  ...
  if [ "$state" = "success" ] || [ "$state" = "failure" ]; then [ "$conclusion" = "$state" ] || { STATUS_SYNC_PENDING=1; return 75; }; fi
  FULL_CHECK_ID="$id"
}
...
post_status() {
  ...
  [ "$POST_STATUS" -eq 1 ] || return 0
  gh api -X POST "repos/$REPO/statuses/$SHA" -f state="$state" -f context="$context" -f description="${description:0:135}" >/dev/null 2>&1 || { STATUS_SYNC_PENDING=1; return 75; }
}
...
publish_success_statuses() {
  local c
  for c in "$CONTEXT_BACKEND" "$CONTEXT_EVAL" "$CONTEXT_FRONTEND" "$CONTEXT_P0" "$CONTEXT_GOV" "$CONTEXT_ARCH" "$CONTEXT_CONT" "$CONTEXT_UI"; do
...
run_governance() {
  ...
  set +e
  if [ -n "$PR" ]; then
    EJC_GOV_SOURCE_ROOT="$GOV_WORKTREE" EJC_PR_NUMBER="$PR" EJC_GOV_REQUIRE_PR=1 bash "$ROOT/scripts/governanca/ci-local-governanca.sh" >"$logfile" 2>&1
  else
    EJC_GOV_SOURCE_ROOT="$GOV_WORKTREE" EJC_GOV_REQUIRE_PR=0 bash "$ROOT/scripts/governanca/ci-local-governanca.sh" >"$logfile" 2>&1
  fi
  rc=$?
  set -e
  return "$rc"
...
revalidate_governance_for_merge() {
  ...
  set +e; run_governance "$tmp"; rc=$?; set -e
  rm -f "$tmp"
  return "$rc"
}
...
refresh_status_from_evidence() {
  ...
  revalidate_governance_for_merge || { post_status failure "$CONTEXT_GOV" "governança atual do PR reprovada" || true; return 1; }
  FULL_CHECK_ID=""
...
attempt_merge() {
  ...
  if printf '%s' "$files_json" | jq -e --arg re "$excecao" '[.[]|select(.filename|test($re))]|length>0' >/dev/null; then return 0; fi
  if printf '%s' "$files_json" | jq -e '[.[]|select(.filename|startswith("backend/alembic/versions/"))]|length>0' >/dev/null; then
    if printf '%s' "$files_json" | jq -e '[.[]|select(.filename|startswith("backend/alembic/versions/"))|select(.patch==null)]|length>0' >/dev/null; then return 0; fi
    if printf '%s' "$files_json" | jq -r '.[]|select(.filename|startswith("backend/alembic/versions/"))|.patch' | grep -Eiq 'op\.drop_|op\.alter_column|DROP (TABLE|COLUMN|INDEX|CONSTRAINT)|TRUNCATE|DELETE FROM'; then return 0; fi
  fi
  verify_merge_snapshot || return 0
  gh api -X PUT "repos/$REPO/pulls/$PR/merge" -f merge_method=squash -f sha="$SHA" >/dev/null 2>&1 || true
}
...
if [ -n "$PR" ] && ! revalidate_governance_for_merge; then post_full_check failure "governança atual do PR reprovada" || true; die "governança mudou"; fi
if [ "$POST_STATUS" -eq 1 ]; then
  publish_success_statuses && full_gate_is_green || STATUS_SYNC_PENDING=1
else
