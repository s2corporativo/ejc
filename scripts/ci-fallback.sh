...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    *) printf 'uso: %s [--pr N|--ref REF] [--merge|--merge-only|--promote-only] [--no-status]\n' "$0" >&2; exit 2 ;;
  esac
done
...
case "$(canon "$ROOT")" in /opt/ejc|/opt/ejc/*) die "recusado em /opt/ejc (produção)";; esac
...
for cmd in git jq flock python3; do command -v "$cmd" >/dev/null 2>&1 || die "$cmd ausente"; done
...
if [ "$POST_STATUS" -eq 1 ]; then gh auth status >/dev/null 2>&1 || die "gh de usuário não autenticado no control plane"; fi

REPO="${EJC_REPO:-}"
if [ -z "$REPO" ] && [ "$POST_STATUS" -eq 1 ]; then REPO="$(gh repo view --json nameWithOwner --jq .nameWithOwner)"; fi
...
if ! flock -n 9; then warn "já existe validação/promoção ativa para $SHA; operação adiada"; exit 75; fi
...
cleanup() {
  set +e
  if [ -n "${GOV_WORKTREE:-}" ] && git worktree list --porcelain | grep -Fqx "worktree $GOV_WORKTREE"; then git worktree remove --force "$GOV_WORKTREE" >/dev/null 2>&1 || true; fi
  if [ "$POST_STATUS" -eq 1 ] && declare -F ejc_github_app_clear >/dev/null 2>&1; then ejc_github_app_clear || true; fi
}
trap cleanup EXIT
trap 'exit 130' INT
...
preflight_app_credential() {
  ...
  result="$(printf '%s' "$payload" | ejc_github_app_gh_api -X POST "repos/$REPO/check-runs" -H 'Accept: application/vnd.github+json' --input - 2>/dev/null)" || die "GitHub App não conseguiu criar Check Run; suíte promovível não iniciada"
...
post_full_check() {
  ...
  case "$state" in
    ...
    success|failure)
      if [ -n "$FULL_CHECK_ID" ]; then payload="$(jq -cn --arg conclusion "$state" --arg summary "${description:0:60000}" '{status:"completed",conclusion:$conclusion,output:{title:"EJC Local Full Gate",summary:$summary}}')"; else payload="$(jq -cn --arg name "$CONTEXT_FULL" --arg sha "$SHA" --arg external_id "$FULL_CHECK_EXTERNAL_ID" --arg conclusion "$state" --arg summary "${description:0:60000}" '{name:$name,head_sha:$sha,status:"completed",conclusion:$conclusion,external_id:$external_id,output:{title:$name,summary:$summary}}')"; fi ;;
    *) die "estado inválido de Check Run: $state" ;;
  esac
  if [ -n "$FULL_CHECK_ID" ] && [ "$state" != pending ]; then
    if ! result="$(printf '%s' "$payload" | ejc_github_app_gh_api -X PATCH "repos/$REPO/check-runs/$FULL_CHECK_ID" -H 'Accept: application/vnd.github+json' --input - 2>/dev/null)"; then STATUS_SYNC_PENDING=1; warn "não foi possível atualizar o Check Run atual para '$state'"; return 75; fi
  else
    if ! result="$(printf '%s' "$payload" | ejc_github_app_gh_api -X POST "repos/$REPO/check-runs" -H 'Accept: application/vnd.github+json' --input - 2>/dev/null)"; then STATUS_SYNC_PENDING=1; warn "não foi possível criar Check Run '$CONTEXT_FULL=$state'; evidência local preservada"; return 75; fi
  fi
  ...
  [[ "$id" =~ ^[1-9][0-9]*$ ]] || { STATUS_SYNC_PENDING=1; warn "Checks API retornou ID inválido"; return 75; }
  [ "$app_id" = "$FALLBACK_APP_ID" ] || { STATUS_SYNC_PENDING=1; warn "Check Run retornado por App divergente"; return 75; }
  [ "$head_sha" = "$SHA" ] || { STATUS_SYNC_PENDING=1; warn "Check Run retornado para SHA divergente"; return 75; }
  [ "$name" = "$CONTEXT_FULL" ] || { STATUS_SYNC_PENDING=1; warn "Check Run retornado com nome divergente"; return 75; }
  [ "$external_id" = "$FULL_CHECK_EXTERNAL_ID" ] || { STATUS_SYNC_PENDING=1; warn "Check Run retornado com external_id divergente"; return 75; }
  if [ "$state" = success ] || [ "$state" = failure ]; then [ "$conclusion" = "$state" ] || { STATUS_SYNC_PENDING=1; warn "conclusão do Check Run divergente"; return 75; }; fi
  FULL_CHECK_ID="$id"
}
...
post_status() {
  ...
  [ "$POST_STATUS" -eq 1 ] || return 0
  if ! gh api -X POST "repos/$REPO/statuses/$SHA" -f state="$state" -f context="$context" -f description="${description:0:135}" >/dev/null 2>&1; then STATUS_SYNC_PENDING=1; warn "não foi possível publicar status informativo '$context=$state'; evidência local preservada"; return 75; fi
}

latest_full_check_id() {
  local checks
  checks="$(ejc_github_app_gh_api --method GET "repos/$REPO/commits/$SHA/check-runs" -H 'Accept: application/vnd.github+json' -f check_name="$CONTEXT_FULL" -F app_id="$FALLBACK_APP_ID" -f filter=latest -F per_page=100 2>/dev/null)" || return 1
...
full_gate_is_green() {
  ...
  printf '%s' "$check" | jq -e --arg name "$CONTEXT_FULL" --arg sha "$SHA" --arg external_id "$FULL_CHECK_EXTERNAL_ID" --argjson app_id "$FALLBACK_APP_ID" '.name==$name and .head_sha==$sha and .external_id==$external_id and .app.id==$app_id and .status=="completed" and .conclusion=="success"' >/dev/null 2>&1 || return 1
  latest="$(latest_full_check_id 2>/dev/null || printf 0)"
  [ "$latest" = "$FULL_CHECK_ID" ]
}
publish_success_statuses() {
  local c
  for c in "$CONTEXT_BACKEND" "$CONTEXT_EVAL" "$CONTEXT_FRONTEND" "$CONTEXT_P0" "$CONTEXT_GOV" "$CONTEXT_ARCH" "$CONTEXT_CONT" "$CONTEXT_UI"; do post_status success "$c" "validado pelo fallback local isolado" || true; done
  post_full_check success "todos os gates locais completos aprovados"
}

local_evidence_is_green() { python3 "$ROOT/scripts/ci_evidence.py" verify --sha-root "$SHA_EVIDENCE" --sha "$SHA" >/dev/null 2>&1; }
finish_attempt() {
...
run_governance() {
  ...
  create_governance_worktree
  set +e
  if [ -n "$PR" ]; then EJC_GOV_SOURCE_ROOT="$GOV_WORKTREE" EJC_PR_NUMBER="$PR" EJC_GOV_REQUIRE_PR=1 bash "$ROOT/scripts/governanca/ci-local-governanca.sh" >"$logfile" 2>&1; else EJC_GOV_SOURCE_ROOT="$GOV_WORKTREE" EJC_GOV_REQUIRE_PR=0 bash "$ROOT/scripts/governanca/ci-local-governanca.sh" >"$logfile" 2>&1; fi
  rc=$?; set -e; return "$rc"
}
revalidate_governance_for_merge() {
  [ -n "$PR" ] || return 1
  local tmp="$STATE_ROOT/governance-revalidate-${SHA:0:12}-$$.log" rc
  set +e; run_governance "$tmp"; rc=$?; set -e; rm -f "$tmp"; return "$rc"
}
refresh_status_from_evidence() {
  [ -n "$PR" ] || return 1
  if ! local_evidence_is_green; then log "Evidência local completa, atual e íntegra do SHA exato ausente; status não promovido."; return 1; fi
  if ! revalidate_governance_for_merge; then post_status failure "$CONTEXT_GOV" "governança atual do PR reprovada" || true; log "Governança atual do PR #$PR reprovada; promoção retida."; return 1; fi
  FULL_CHECK_ID=""; FULL_CHECK_EXTERNAL_ID="ejc-fallback:$SHA:promotion:$(date +%s)-$$"
  if ! publish_success_statuses; then log "Evidência local verde, mas Check Run canônico não sincronizou; merge retido."; return 1; fi
  if ! full_gate_is_green; then log "Check Run exato/mais recente desta promoção não ficou verde; merge retido."; return 1; fi
}

verify_branch_protection_for_merge() {
  local protection
  protection="$(gh api "repos/$REPO/branches/main/protection" 2>/dev/null)" || return 1
  printf '%s' "$protection" | jq -e --arg name "$CONTEXT_FULL" --argjson app_id "$FALLBACK_APP_ID" '.required_status_checks.strict==true and any(.required_status_checks.checks[]?; .context==$name and .app_id==$app_id) and .enforce_admins.enabled==true and (.required_pull_request_reviews.required_approving_review_count//0)>=1 and .required_pull_request_reviews.require_code_owner_reviews==true and .required_pull_request_reviews.require_last_push_approval==true and .required_conversation_resolution.enabled==true and .required_linear_history.enabled==true and .allow_force_pushes.enabled==false and .allow_deletions.enabled==false' >/dev/null 2>&1
}
verify_merge_snapshot() {
  ...
  review_decision="$(printf '%s' "$info" | jq -r '.reviewDecision // ""')"; [ "$review_decision" = APPROVED ] || return 1
  labels="$(printf '%s' "$info" | jq -r '[.labels[].name]|join(",")')"; case ",$labels," in *,retencao-humana,*) return 1;; esac
}
...
attempt_merge() {
  ...
  if ! files_json="$(gh api --paginate "repos/$REPO/pulls/$PR/files?per_page=100" 2>/dev/null | jq -s 'add')"; then log "lista/diff de arquivos do PR indisponível; merge retido fail-closed"; return 0; fi
  ...
  if printf '%s' "$files_json" | jq -e --arg re "$excecao" '[.[]|select(.filename|test($re))]|length>0' >/dev/null; then log "PR #$PR toca caminho de exceção §6-A; validação concluída, merge retido."; return 0; fi
  if printf '%s' "$files_json" | jq -e '[.[]|select(.filename|startswith("backend/alembic/versions/"))]|length>0' >/dev/null; then
    if printf '%s' "$files_json" | jq -e '[.[]|select(.filename|startswith("backend/alembic/versions/"))|select(.patch==null)]|length>0' >/dev/null; then log "migration com patch indisponível/truncado; merge retido fail-closed"; return 0; fi
    if printf '%s' "$files_json" | jq -r '.[]|select(.filename|startswith("backend/alembic/versions/"))|.patch' | grep -Eiq 'op\.drop_|op\.alter_column|DROP (TABLE|COLUMN|INDEX|CONSTRAINT)|TRUNCATE|DELETE FROM'; then log "PR #$PR contém migration potencialmente destrutiva; merge retido."; return 0; fi
  fi
  verify_merge_snapshot || { log "estado mudou imediatamente antes do merge; retido"; return 0; }
  set +e
  local merge_json rc merged message
  merge_json="$(gh api -X PUT "repos/$REPO/pulls/$PR/merge" -f merge_method=squash -f sha="$SHA" 2>&1)"; rc=$?; set -e
  if [ "$rc" -ne 0 ]; then log "PR #$PR ainda não elegível ao merge (review/protection/conflito); será reavaliado sem repetir CI do mesmo SHA."; return 0; fi
  merged="$(printf '%s' "$merge_json" | jq -r '.merged // false' 2>/dev/null || echo false)"; message="$(printf '%s' "$merge_json" | jq -r '.message // ""' 2>/dev/null || true)"
  if [ "$merged" = true ]; then log "PR #$PR integrado após fallback local completo."; else log "PR #$PR não integrado: ${message:-proteção/review pendente}."; fi
}
...
run_worker_stage() {
  ...
  set +e
  EJC_CI_WORKER_USER="${EJC_CI_WORKER_USER:-}" EJC_CI_WORKER_ROOT="${EJC_CI_WORKER_ROOT:-/var/tmp/ejc-ci-worker}" EJC_FALLBACK_APP_PRIVATE_KEY_FILE="${EJC_FALLBACK_APP_PRIVATE_KEY_FILE:-}" EJC_CI_STATE_ROOT="$STATE_ROOT" bash "$ROOT/scripts/ci-worker-isolation.sh" run-stage --sha "$SHA" --stage "$key" >"$logfile" 2>&1
  rc=$?; set -e
  if [ "$rc" -ne 0 ]; then finish_attempt failure "$key" "$rc" 0 || true; post_status failure "$context" "fallback local falhou: $key (exit $rc)" || true; post_full_check failure "fallback local falhou: $key (exit $rc)" || true; tail -n 120 "$logfile" >&2 || true; return "$rc"; fi
}
run_governance_stage() {
  local logfile="$ATTEMPT_DIR/governanca.log" rc
  log "Executando governança trusted contra snapshot do PR…"
  set +e; run_governance "$logfile"; rc=$?; set -e
  if [ "$rc" -ne 0 ]; then finish_attempt failure governanca "$rc" 0 || true; post_status failure "$CONTEXT_GOV" "governança falhou (exit $rc)" || true; post_full_check failure "governança falhou (exit $rc)" || true; tail -n 120 "$logfile" >&2 || true; return "$rc"; fi
}

preflight_app_credential
if [ "$PROMOTE_ONLY" -eq 1 ]; then [ -n "$PR" ] || die "--promote-only exige --pr"; refresh_status_from_evidence; exit $?; fi
if [ "$MERGE_ONLY" -eq 1 ]; then [ -n "$PR" ] || die "--merge-only exige --pr"; attempt_merge; exit 0; fi
...
for c in "$CONTEXT_BACKEND" "$CONTEXT_EVAL" "$CONTEXT_FRONTEND" "$CONTEXT_P0" "$CONTEXT_GOV" "$CONTEXT_ARCH" "$CONTEXT_CONT" "$CONTEXT_UI"; do post_status pending "$c" "CI local isolado em execução" || true; done
...
if [ -n "$PR" ] && ! revalidate_governance_for_merge; then post_status failure "$CONTEXT_GOV" "governança atual do PR reprovada" || true; post_full_check failure "governança atual do PR reprovada" || true; die "governança do PR mudou/reprovou após a suíte"; fi
if [ "$POST_STATUS" -eq 1 ]; then
  if publish_success_statuses && full_gate_is_green; then log "Fallback completo aprovado para $SHA. Evidência local atual: $SHA_EVIDENCE/latest-success.json"; else STATUS_SYNC_PENDING=1; warn "suíte local aprovada, mas Check Run canônico não sincronizou; merge permanece bloqueado"; fi
