...
describe("moduleRegistry", () => {
  ...
  it("registra o DPT Empresarial 360 como workspace essencial sem alargar RBAC", () => {
    ...
    for (const role of ["advogado_auxiliar", "estagiario", "financeiro", "secretaria", "cliente_externo"]) {
  ...
  it("oculta módulos internos, legados e não autorizados do menu efetivo", () => {
    ...
    expect(advogado.some((module) => module.key === "knowledge-hub")).toBe(false);
  });
  ...
  it("nega financeiro em Peças — espelha o backend (Issue #694, allowlist EQUIPE_JURIDICA em POST /pecas/gerar)", () => {
    ...
    expect(getProductionNavigation("financeiro").some((m) => m.path === "/pecas")).toBe(false);
  });

  it("destaca a Sala Jurídica e o Financeiro apenas para os perfis autorizados", () => {
    const advogado = getProductionNavigation("advogado");
    const socio = getProductionNavigation("socio");
    expect(advogado.find((item) => item.path === "/sala-juridica")?.essential).toBe(true);
  ...
  it("preserva redirecionamentos das duplicidades consolidadas", () => {
    const map = new Map(LEGACY_REDIRECTS.map((redirect) => [redirect.from, redirect.to]));
    expect(map.get("/partner-withdrawals")).toBe("/financeiro?tab=societaria&sub=saques");
    ...
    expect(map.get("/central-relacionamento")).toBe("/atividades?tab=relacionamento");
  ...
  it("mantém o menu enxuto e o modo essencial por perfil", () => {
    for (const role of ["superadmin", "admin", "socio", "advogado"]) {
      ...
      expect(getProductionNavigation(role).some((m) => m.path === "/ferramentas")).toBe(true);
    }
    const navegacaoAdvogado = getProductionNavigation("advogado");
    const advogado = navegacaoAdvogado.map((m) => m.path);
    const essenciais = navegacaoAdvogado.filter((m) => m.essential).map((m) => m.path);
...
describe("radar consolidado", () => {
  ...
  it("herda o RBAC mais restritivo dos dois — a fusão não alarga acesso", () => {
    ...
    for (const papel of ["advogado_auxiliar", "estagiario", "financeiro", "secretaria"]) {
