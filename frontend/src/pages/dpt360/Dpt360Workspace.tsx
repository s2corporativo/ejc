  ...
  return (
    <Card className="min-h-[132px]">
      ...
      <p className="mt-2 text-xs leading-5 text-slate-500 dark:text-slate-400">
        {note ||
          (value === null
            ? "Integração especializada ainda não ativada."
            : "Dados reais do EJC.")}
      </p>
    </Card>
  );
    ...
    <div className="mb-4 space-y-2">
      {data.degraded.length > 0 && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-400/20 dark:bg-amber-400/10 dark:text-amber-100">
          <strong>Visão parcialmente indisponível:</strong>{" "}
          {data.degraded.join(", ")}. Os demais blocos continuam usando somente
          dados confirmados.
        </div>
      )}
      {data.truncated.length > 0 && (
        <div className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600 dark:border-white/10 dark:bg-white/[0.04] dark:text-slate-300">
          <strong>Cobertura parcial:</strong> {data.truncated.join(", ")}{" "}
          possuem mais de 100 registros visíveis. Antes da produção definitiva,
          o dashboard deverá migrar para endpoint agregador paginado.
        </div>
      )}
    </div>
  );
...
function ExecutiveHome({ data }: { data: DptData }) {
  ...
  const businessCases = useMemo(
    () =>
      data.cases.filter(
        (item) => companyById.has(item.client_id) && isBusinessCase(item),
      ),
  ...
  const priorities = useMemo(() => {
    ...
    const deadlineItems = upcoming.map((deadline) => {
      const linkedCase = deadline.case_id
        ? caseById.get(deadline.case_id)
        : undefined;
      const company = linkedCase
        ? companyById.get(linkedCase.client_id)
        : undefined;
      const overdue =
        deadline.data_prazo < today || deadline.status === "vencido";
      ...
      return {
        ...
        tone: overdue
          ...
          : dueToday
            ...
            : "bg-amber-400",
        href: linkedCase
          ? `/atividades?tipo=prazo&caso=${linkedCase.id}`
    ...
    <div className="space-y-5">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-6">
        <Metric
          ...
          note="Clientes pessoa jurídica visíveis na sua carteira."
        />
        <Metric
          label="Riscos críticos"
          value={critical.length}
          note="Casos empresariais abertos com prioridade crítica ou risco alto registrado."
        />
        <Metric
          label="Providências próximas"
          value={upcoming.length}
          note="Prazos pendentes/vencidos ligados a casos empresariais até os próximos 7 dias."
        />
        <Metric
          label="Mudanças jurídicas hoje"
          value={null}
          note="Será conectado ao Radar DPT após a estabilização das fontes oficiais."
        />
        <Metric
          label="Empresas potencialmente impactadas"
          value={null}
          note="Depende do Motor de Impacto e do Perfil Jurídico Vivo."
        />
        <Metric
          label="Diagnósticos pendentes"
          value={null}
          note="Conceito ainda sem persistência própria confirmada; não é tratado como zero."
        />
      </div>
        ...
        <Card>
          ...
          {priorities.length === 0 ? (
            <div className="mt-5 rounded-xl border border-dashed border-slate-200 px-4 py-7 text-sm text-slate-500 dark:border-white/10 dark:text-slate-400">
              Nenhum prazo próximo ou caso com sinal crítico foi encontrado nos
              dados disponíveis. Isso não equivale a regularidade jurídica.
            </div>
          ) : (
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-slate-500 dark:text-slate-400">
                O cruzamento DPT específico entre carteira, fontes, vigência,
                fatos e HITL será ligado somente após as frentes de RAG, fontes
                e crítica adversarial. Nesta onda, nenhuma análise autônoma é
                simulada.
              </p>
              <Link
                to="/inteligencia"
                className="mt-4 inline-flex items-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-slate-800 dark:bg-white dark:text-slate-950"
              >
      ...
      <Card>
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          >
            Abrir radar atual <ChevronRight className="h-3.5 w-3.5" />
          </Link>
        </div>
          ...
          {[
            ...
            >
              ...
              <div className="mt-1 text-[11px] text-slate-400">
                Aguardando motor DPT de relevância e impacto.
              </div>
            </div>
          ))}
        </div>
  ...
  if (!summaries.length) {
    return (
      <Empty
        ...
        descricao="O DPT 360 utiliza Clientes pessoa jurídica já cadastrados no EJC. Nenhum cadastro paralelo será criado."
      />
    );
  }
  return (
        ...
        <table className="min-w-full text-left text-sm">
          <thead className="border-b border-slate-100 text-xs uppercase tracking-wide text-slate-400 dark:border-white/10">
            <tr>
              ...
              <th className="px-3 py-3">Riscos</th>
              ...
              <tr key={company.id}>
                ...
                <td className="px-3 py-4 text-right">
                  <Link
                    ...
                    className="inline-flex items-center gap-1 text-xs font-semibold text-amber-700 hover:text-amber-800 dark:text-amber-300"
                  >
...
function CompanyDetail({
  ...
  const summary = useMemo(
    () =>
      buildCompanySummaries(data).find((item) => item.company.id === clientId),
    [clientId, data],
  );
  if (!summary)
    return (
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
              ...
              · Perfil Jurídico Vivo será enriquecido progressivamente sem
              duplicar o cadastro de Cliente.
            </p>
          </div>
          <Link
            to={`/clientes/${company.id}`}
            className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-700 hover:border-amber-300 hover:text-amber-800 dark:border-white/10 dark:text-slate-200"
          >
        ...
        {HEALTH_AREAS.map((area) => (
          <Card key={area} className="p-4">
            ...
            <div className="mt-2 text-xs leading-5 text-slate-400">
              Sem diagnóstico especializado aprovado e evidenciado.
            </div>
          </Card>
        ))}
      </div>
...
function CasesView({ data }: { data: DptData }) {
  ...
  const cases = data.cases.filter(
    (item) => ids.has(item.client_id) && isBusinessCase(item),
  );
  if (!cases.length)
    return (
      <Empty
        titulo="Nenhum caso empresarial encontrado"
        descricao="A visão usa os Casos canônicos do EJC e filtra somente clientes pessoa jurídica e áreas empresariais relacionadas."
      />
    );
  return (
    <Card>
      ...
      <div className="space-y-2">
        {cases.map((item) => (
          <Link
            key={item.id}
            to={`/casos/${item.id}`}
            className="flex flex-col gap-2 rounded-xl border border-slate-100 p-4 transition hover:border-amber-200 hover:bg-amber-50/30 dark:border-white/10 dark:hover:bg-white/[0.03] sm:flex-row sm:items-center sm:justify-between"
          >
            <div>
              <div className="font-semibold text-slate-900 dark:text-slate-100">
                {item.titulo}
              ...
              <div className="mt-1 text-xs text-slate-400">
                {names.get(item.client_id)
                  ? companyDisplayName(names.get(item.client_id)!)
                  : "Empresa"}{" "}
                · {item.area} · {item.status}
              </div>
            </div>
...
> = {
  diagnostico: {
    title: "Diagnóstico Jurídico Empresarial 360",
    text: "O fluxo fatos → evidências → questões → fontes → riscos → providências → HITL será persistido somente após auditoria do schema. Nesta onda não é criado diagnóstico fictício nem migration concorrente.",
    icon: ClipboardCheck,
  },
  radar: {
    title: "Radar Jurídico DPT",
    text: "A camada de impacto por empresa dependerá das fontes oficiais, vigência e quarentena já em estabilização. O radar atual do EJC continua sendo a fonte operacional até esse acoplamento.",
    href: "/radar",
    hrefLabel: "Abrir Radar atual",
    icon: Radar,
  },
  inteligencia: {
    title: "Inteligência Jurídica Empresarial",
    text: "O DPT reutilizará o núcleo central de IA. O Modo Conselho e a pergunta executiva serão adicionados como competências do orchestrator após HITL/citações/crítica adversarial estarem consolidados.",
    href: "/inteligencia",
    hrefLabel: "Abrir Pesquisa e IA",
    icon: BrainCircuit,
  },
  ferramentas: {
    title: "Ferramentas Empresariais",
    text: "Os workspaces jurídicos continuam canônicos. O DPT 360 fará composição e atalhos, sem duplicar calculadoras ou analisadores já existentes.",
    href: "/areas-de-atuacao",
    hrefLabel: "Abrir Áreas de Atuação",
    icon: Wrench,
  },
  obrigacoes: {
    title: "Agenda de Obrigações Empresariais",
    text: "Licenças, TACs, contratos, políticas e obrigações recorrentes podem exigir persistência própria. O modelo será criado somente se a auditoria confirmar que Prazos/Tarefas não atendem semanticamente ao conceito.",
    href: "/atividades",
    hrefLabel: "Abrir Agenda e Prazos",
    icon: CalendarClock,
  },
  biblioteca: {
    title: "Biblioteca Empresarial",
    text: "Legislação, jurisprudência, teses internas, modelos e lições aprendidas continuarão no Conhecimento canônico, com segregação entre conteúdo público, escritório, cliente e caso.",
  ...
  relatorios: {
    ...
    text: "O relatório mensal será gerado como rascunho com atividades, riscos, prazos, mudanças relevantes e recomendações. Nenhum envio automático ao cliente será permitido sem revisão e aprovação humana.",
    icon: BookOpen,
  },
};
  ...
  return (
    <Card className="min-h-[360px]">
      ...
      <div className="mt-6 rounded-xl border border-dashed border-slate-200 bg-slate-50/60 p-4 text-sm text-slate-500 dark:border-white/10 dark:bg-white/[0.03] dark:text-slate-400">
        ...
        arquitetura reservada, sem dados simulados e sem novo schema.
      </div>
      {item.href && (
        <Link
          to={item.href}
          className="mt-5 inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-sm font-semibold text-slate-700 hover:border-amber-300 hover:text-amber-800 dark:border-white/10 dark:text-slate-200"
        >
          {item.hrefLabel}
...
export default function Dpt360Workspace() {
  ...
  const load = useCallback(async () => {
    ...
    const [clientsResult, casesResult, deadlinesResult] =
    ...
    if (deadlinesResult.status === "fulfilled") {
      const collection = extractCollection<Deadline>(
  ...
  return (
    <div className="mx-auto w-full max-w-[1600px] space-y-4 pb-8">
      <div className="overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-sm dark:border-white/10 dark:bg-slate-950/50">
        <div className="border-b border-slate-100 bg-gradient-to-r from-slate-950 via-slate-900 to-slate-800 px-5 py-5 text-white dark:border-white/10 sm:px-7">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              <span className="grid h-12 w-12 place-items-center rounded-2xl border border-amber-300/20 bg-white/5 text-amber-300">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-slate-300">
                  De Paula Teixeira Advogados · subaplicativo do EJC
                </p>
              </div>
            </div>
            <button
              type="button"
              onClick={() => void load()}
              className="inline-flex items-center gap-2 rounded-xl border border-white/15 bg-white/5 px-3 py-2 text-xs font-semibold text-slate-200 transition hover:bg-white/10"
            >
              <RefreshCcw
        ...
        <nav
          className="flex gap-1 overflow-x-auto border-b border-slate-100 px-3 py-2 dark:border-white/10"
          aria-label="Navegação DPT Empresarial 360"
        >
          {NAV_ITEMS.map((item) => {
            const Icon = item.icon;
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) =>
                  `inline-flex shrink-0 items-center gap-2 rounded-xl px-3 py-2 text-xs font-semibold transition ${isActive ? "bg-slate-950 text-white dark:bg-white dark:text-slate-950" : "text-slate-500 hover:bg-slate-100 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-white/[0.06] dark:hover:text-white"}`
                }
              >
      ...
      {fatal ? (
        <ErrorState
          message="Não foi possível carregar Clientes, Casos e Prazos do EJC para o DPT 360."
          onRetry={() => void load()}
        />
      ) : loading ? (
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <div className="rounded-2xl border border-slate-200/80 bg-white p-4 dark:border-white/10 dark:bg-slate-950/40">
          ...
          <p className="mt-2 text-xs leading-5 text-slate-500 dark:text-slate-400">
            Arquitetura prevista para representar empresa, contratos,
            obrigações, licenças, dados, riscos, eventos e fontes aplicáveis sem
            duplicar os registros canônicos.
          </p>
        </div>
        <div className="rounded-2xl border border-slate-200/80 bg-white p-4 dark:border-white/10 dark:bg-slate-950/40">
          <div className="flex items-center gap-2 text-sm font-semibold text-slate-900 dark:text-slate-100">
          ...
          <p className="mt-2 text-xs leading-5 text-slate-500 dark:text-slate-400">
            Planejado para detectar fatos sem prova, citações não confirmadas,
            vigência duvidosa, datas conflitantes e divergências entre peça e
            cadastro.
          </p>
        </div>
        <div className="rounded-2xl border border-slate-200/80 bg-white p-4 dark:border-white/10 dark:bg-slate-950/40">
          <div className="flex items-center gap-2 text-sm font-semibold text-slate-900 dark:text-slate-100">
            ...
            Modo Conselho Jurídico
          </div>
          <p className="mt-2 text-xs leading-5 text-slate-500 dark:text-slate-400">
            Camada executiva futura para priorizar riscos e providências com
            base em evidências, fontes e decisão humana.
          </p>
        </div>
        <div className="rounded-2xl border border-slate-200/80 bg-white p-4 dark:border-white/10 dark:bg-slate-950/40">
          <div className="flex items-center gap-2 text-sm font-semibold text-slate-900 dark:text-slate-100">
          ...
          <p className="mt-2 text-xs leading-5 text-slate-500 dark:text-slate-400">
            Decisões profissionais aprovadas serão versionadas no conhecimento
            interno; nenhuma preferência será inferida como regra sem aprovação.
          </p>
        </div>
      </div>
    </div>
