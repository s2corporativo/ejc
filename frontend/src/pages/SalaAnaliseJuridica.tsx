...
const stringify = (value: unknown): string => {
  ...
  if (typeof value === "object") {
    ...
    const preferred = item.fato ?? item.descricao ?? item.texto ?? item.titulo ?? item.nome ?? item.valor;
...
export default function SalaAnaliseJuridica() {
  ...
  const loadList = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await api.get("/raio-x/", { params: { page_size: 100 } });
      const list = Array.isArray(data) ? data : data?.data || [];
      setItems(list);
      setSelected((current) => {
        if (!current) return list[0] || null;
        return list.find((item: Analise) => item.id === current.id) || list[0] || null;
      });
    } catch (err: any) {
      setError(err?.response?.data?.detail || "Não foi possível carregar as análises.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const createAnalysis = async () => {
    ...
    try {
      ...
      setError(err?.response?.data?.detail || "Não foi possível criar a análise.");
    } finally {
      setBusy(false);
    }
  };
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const content =
        data?.resposta || data?.analise || data?.texto || data?.resultado || JSON.stringify(data, null, 2);
      ...
      const detail = err?.response?.data?.detail || "A análise jurídica não pôde ser concluída.";
  ...
  const uploadFiles = async (files: FileList | null) => {
    ...
    try {
      ...
      const systemMessage: ChatMessage = {
        ...
        role: "assistant",
        content:
          `${files.length} arquivo(s) recebido(s) e processado(s). O painel de fatos, contradições, riscos e pendências foi atualizado.`,
        created_at: new Date().toISOString(),
      };
  ...
  const openConversion = async () => {
    ...
    setError(null);
    try {
      const { data } = await api.get(`/raio-x/${selected.id}/conversao/preview`);
      setPreview(data);
      ...
      setError(err?.response?.data?.detail || "Não foi possível preparar a conversão.");
    } finally {
      setBusy(false);
    }
  };
    ...
    try {
      ...
      const payload = {
        ...
        caso: {
          ...
          descricao_fatos:
            selected.relatorio?.sintese_executiva || messages.map((m) => `${m.role}: ${m.content}`).join("\n\n"),
          case_type: "judicial",
        },
      ...
      const { data } = await api.post(`/raio-x/${selected.id}/converter`, payload);
      const caseId = data?.case_id || data?.caso?.id || data?.id;
      if (caseId) navigate(`/casos/${caseId}`);
      else await refreshSelected(selected.id);
    } catch (err: any) {
      setError(err?.response?.data?.detail || "A conversão não pôde ser concluída.");
    } finally {
      setBusy(false);
    }
  };
    ...
    <div className="flex h-[calc(100vh-5.5rem)] min-h-[720px] flex-col gap-4">
      ...
      {error && (
        <div className="flex items-start justify-between rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800">
          <span>{error}</span>
          <button onClick={() => setError(null)} aria-label="Fechar"><X size={16} /></button>
        </div>
      )}
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm xl:grid-cols-[280px_minmax(0,1fr)_340px]">
        <aside className="flex min-h-0 flex-col border-b border-slate-200 bg-slate-50/70 xl:border-b-0 xl:border-r">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto p-2">
            {loading ? (
              ...
              <EmptyState icon={Scale} title="Nenhuma análise" description="Crie uma sala para iniciar a investigação jurídica." />
            ) : (
        ...
        <main className="flex min-h-0 flex-col bg-white">
          {!selected ? (
            <div className="flex h-full items-center justify-center p-8">
              <EmptyState icon={Bot} title="Selecione ou crie uma análise" description="A conversa ficará vinculada ao dossiê preliminar e às provas anexadas." />
            </div>
          ) : (
            <>
              <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 px-5 py-4">
                <div>
                  ...
                  <div className="mt-1 flex flex-wrap gap-2 text-xs text-slate-500">
                    ...
                    {selected.risco_nivel && <Badge tone={selected.risco_nivel === "critico" ? "red" : "amber"}>{selected.risco_nivel}</Badge>}
                  </div>
                </div>
                ...
                <div className="mx-auto max-w-3xl space-y-5">
                  ...
                  {messages.map((message) => (
                    <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                      <div className={`max-w-[88%] rounded-2xl px-4 py-3 text-sm shadow-sm ${message.role === "user" ? "bg-slate-900 text-white" : "border border-slate-200 bg-white text-slate-800"}`}>
                        ...
                        <div className={`mt-2 text-[10px] ${message.role === "user" ? "text-slate-400" : "text-slate-400"}`}>
                  ...
                  {busy && (
                    <div className="flex items-center gap-2 text-sm text-slate-500">
                      <Loader2 className="animate-spin" size={16} /> Analisando fatos e provas…
                    </div>
                  )}
                  <div ref={bottomRef} />
                </div>
              ...
              <footer className="border-t border-slate-200 bg-white p-4">
                ...
                <p className="mx-auto mt-2 max-w-3xl text-center text-[11px] text-slate-400">
                  Saída preliminar sujeita à revisão do advogado. Hipóteses e fontes não verificadas devem ser confirmadas antes do uso externo.
                </p>
              </footer>
            </>
          )}
        ...
        <aside className="min-h-0 overflow-y-auto border-t border-slate-200 bg-slate-50/70 p-4 xl:border-l xl:border-t-0">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h3 className="font-semibold text-slate-900">Estado atual da análise</h3>
          </div>
          {!selected ? (
            ...
            <div className="space-y-4">
              ...
              {!!selected.documentos?.length && (
                <div className="rounded-xl border border-slate-200 bg-white p-3">
                  <div className="mb-2 flex items-center gap-2 text-sm font-semibold"><FileText size={15} /> Provas anexadas</div>
                  <ul className="space-y-1 text-xs text-slate-600">
                    {selected.documentos.map((doc) => <li key={doc.id} className="truncate">• {doc.nome_original}</li>)}
                  </ul>
                </div>
              )}
              <Button variant="secondary" className="w-full" onClick={() => void api.post(`/raio-x/${selected.id}/arquivar`).then(loadList)}>
      ...
      {showCreate && (
        <Modal title="Nova análise preliminar" onClose={() => setShowCreate(false)}>
          ...
          <input className="mt-1 w-full rounded-lg border p-2" value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder="Ex.: Acidente da Hilux — DF-345" />
          <label className="mt-4 block text-sm font-medium">Cliente ou interessado</label>
          <input className="mt-1 w-full rounded-lg border p-2" value={newClient} onChange={(e) => setNewClient(e.target.value)} placeholder="Opcional" />
      ...
      {showConvert && selected && (
        <Modal title="Transformar análise em caso" onClose={() => setShowConvert(false)} wide>
          <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">
            Confira os dados antes de criar o cadastro oficial. A conversa original permanecerá preservada para auditoria.
          </div>
          {preview?.bloqueia && (
            <div className="mt-3 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-800">
              A prévia identificou impedimentos. Revise conflitos e duplicidades antes de confirmar.
            </div>
          )}
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <div>
              <label className="text-sm font-medium">Cliente</label>
              <select className="mt-1 w-full rounded-lg border p-2" value={conversion.clientMode} onChange={(e) => setConversion({ ...conversion, clientMode: e.target.value })}>
                <option value="novo">Criar novo cliente</option>
                <option value="existente">Usar cliente existente</option>
              </select>
            </div>
            {conversion.clientMode === "existente" ? (
              <div><label className="text-sm font-medium">ID do cliente</label><input className="mt-1 w-full rounded-lg border p-2" value={conversion.clientId} onChange={(e) => setConversion({ ...conversion, clientId: e.target.value })} /></div>
            ) : (
              <div><label className="text-sm font-medium">Nome do cliente</label><input className="mt-1 w-full rounded-lg border p-2" value={conversion.clientName} onChange={(e) => setConversion({ ...conversion, clientName: e.target.value })} /></div>
            )}
            {conversion.clientMode === "novo" && (
              <div><label className="text-sm font-medium">CPF/CNPJ</label><input className="mt-1 w-full rounded-lg border p-2" value={conversion.clientCpf} onChange={(e) => setConversion({ ...conversion, clientCpf: e.target.value })} /></div>
            )}
            <div><label className="text-sm font-medium">Título do caso</label><input className="mt-1 w-full rounded-lg border p-2" value={conversion.caseTitle} onChange={(e) => setConversion({ ...conversion, caseTitle: e.target.value })} /></div>
            <div><label className="text-sm font-medium">Área jurídica</label><select className="mt-1 w-full rounded-lg border p-2" value={conversion.area} onChange={(e) => setConversion({ ...conversion, area: e.target.value })}>{AREAS.map((area) => <option key={area} value={area}>{area.replaceAll("_", " ")}</option>)}</select></div>
          </div>
          <div className="mt-4 space-y-2 text-sm">
            <label className="flex items-center gap-2"><input type="checkbox" checked={conversion.confirmDuplicate} onChange={(e) => setConversion({ ...conversion, confirmDuplicate: e.target.checked })} /> Conferi possíveis duplicidades.</label>
            <label className="flex items-center gap-2"><input type="checkbox" checked={conversion.confirmConflict} onChange={(e) => setConversion({ ...conversion, confirmConflict: e.target.checked })} /> Conferi possíveis conflitos de interesse.</label>
          </div>
          <div className="mt-5 flex justify-end gap-2">
            <Button variant="secondary" onClick={() => setShowConvert(false)}>Cancelar</Button>
            <Button onClick={() => void convertToCase()} disabled={busy || !conversion.caseTitle.trim() || (conversion.clientMode === "novo" ? !conversion.clientName.trim() : !conversion.clientId.trim())}>
...
function Panel({ title, icon: Icon, items, tone = "slate" }: { title: string; icon: typeof FileText; items?: unknown[]; tone?: "slate" | "amber" | "red" }) {
  ...
  return (
    <div className={`rounded-xl border ${border} bg-white p-3`}>
      ...
      {!items?.length ? <p className="text-xs text-slate-400">Nenhum item identificado com segurança.</p> : <ul className="space-y-2 text-xs text-slate-600">{items.slice(0, 8).map((item, index) => <li key={index} className="rounded-lg bg-slate-50 p-2">{stringify(item)}</li>)}</ul>}
    </div>
  );
}
...
function Modal({ title, children, onClose, wide = false }: { title: string; children: React.ReactNode; onClose: () => void; wide?: boolean }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4" onMouseDown={onClose}>
      <div className={`max-h-[90vh] w-full overflow-y-auto rounded-2xl bg-white p-5 shadow-2xl ${wide ? "max-w-3xl" : "max-w-lg"}`} onMouseDown={(e) => e.stopPropagation()}>
        <div className="mb-4 flex items-center justify-between"><h2 className="text-lg font-semibold">{title}</h2><button onClick={onClose}><X size={20} /></button></div>
        {children}
      </div>
    </div>
  );
