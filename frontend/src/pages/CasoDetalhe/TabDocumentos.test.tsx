...
describe("TabDocumentos — vínculo canônico", () => {
  ...
  it("busca candidatos no endpoint do caso e vincula por POST de domínio", async () => {
    ...
    expect(screen.getByText(/08\/08\/2026/)).toBeTruthy();
  ...
  it("não renderiza vínculo para papel rejeitado pelo backend", async () => {
    ...
    expect(
      screen.queryByPlaceholderText("Buscar documento por título ou arquivo…"),
    ).toBeNull();
    expect(
      get.mock.calls.some(([url]) =>
        String(url).includes("/documentos/candidatos"),
      ),
    ).toBe(false);
  });
  ...
  it("não oferece movimento se uma resposta inesperada trouxer documento de outro caso", async () => {
    ...
    expect(
      screen.getByText(/evidência original não pode ser movida/i),
    ).toBeTruthy();
  ...
  it("limpa candidatos do contexto anterior assim que o caso muda", async () => {
    vi.spyOn(api, "get").mockImplementation((url) => {
      const texto = String(url);
      if (texto === "/cases/case-1/documentos/candidatos") {
        return Promise.resolve({
          data: {
            data: [{ id: "antigo", titulo: "Documento do caso anterior" }],
            total: 1,
          },
        });
      }
      if (texto === "/cases/case-2/documentos/candidatos") {
        return Promise.resolve({ data: { data: [], total: 0 } });
      }
      return Promise.resolve({ data: { data: [] } });
    });

    const { rerender } = render(<TabDocumentos caseId="case-1" />);
    fireEvent.change(
      screen.getByPlaceholderText("Buscar documento por título ou arquivo…"),
      { target: { value: "documento" } },
    );
    ...
    expect(screen.getByText("Documento do caso anterior")).toBeTruthy();

    rerender(<TabDocumentos caseId="case-2" />);
    expect(screen.queryByText("Documento do caso anterior")).toBeNull();
