...
function Lista({ itens }: { itens?: unknown[] }) {
  if (!itens?.length) return <p className="text-sm text-slate-500">Nenhum item identificado.</p>;
  return (
    <ul className="space-y-2">
      {itens.map((item, index) => (
        <li key={`${rotulo(item)}-${index}`} className="text-sm text-slate-700 dark:text-slate-200">
...
function Secao({ title, children, open = false }: { title: string; children: React.ReactNode; open?: boolean }) {
  return (
    <details open={open} className="rounded-xl border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-900">
...
export default function DossieJuridico({ caseId, onNovo }: DossieProps) {
  ...
  const [preparandoPeca, setPreparandoPeca] = useState(false);
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      const { data } = await api.post(`/entrada/casos/${caseId}/dossie-juridico`);
      setDossie(data);
    } catch (err) {
      setErro(textoErro(err, "Não foi possível gerar o dossiê jurídico."));
    } finally {
      setLoading(false);
    }
  }, [caseId]);
  ...
  const aprovarPlano = async () => {
    ...
    try {
      ...
      toast.error(textoErro(err, "Não foi possível aprovar o plano jurídico."));
    } finally {
      setAprovando(false);
    }
  };
  ...
  const prepararPeca = async () => {
    ...
    try {
      ...
      setGatePeca(textoErro(err, "Não foi possível preparar o Motor de Peça."));
    } finally {
      setPreparandoPeca(false);
    }
  };

  const gerarPeca = async () => {
    const codigo = planoPeca?.peca_principal;
    if (!codigo) {
      setGatePeca("Selecione ou confirme uma peça cabível no Motor de Peça.");
      return;
    }
    if (!termoInicial || !termoConfirmado) {
      setGatePeca("Informe e confirme o termo inicial antes de gerar a peça. O EJC não presume prazo fatal.");
      return;
    }
    setGerandoPeca(true);
    setGatePeca(null);
    try {
      ...
      setGatePeca(textoErro(err, "O Motor de Peça bloqueou a geração. Revise os requisitos."));
    } finally {
  ...
  if (erro || !dossie) {
    return (
      <div className="mx-auto max-w-4xl space-y-4 py-4">
        <Alert variant="danger" title="Dossiê indisponível">{erro || "Resposta vazia."}</Alert>
        <Button variant="secondary" onClick={() => void carregar()} icon={<RefreshCw className="h-4 w-4" />}>
          Tentar novamente
        </Button>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-5 py-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-xl font-semibold text-slate-950 dark:text-white">Dossiê Jurídico</h2>
            <Badge tone="purple">rascunho</Badge>
          </div>
          <p className="mt-1 max-w-3xl text-sm text-slate-500">{dossie.aviso}</p>
        </div>
        <div className="flex gap-2">
          {onNovo && <Button variant="ghost" onClick={onNovo}>Nova entrada</Button>}
          <Button variant="secondary" onClick={() => navigate(`/casos/${caseId}`)}>Abrir caso</Button>
        </div>
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <Card className="p-4">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            {tipos.map((tipo) => <Badge key={tipo} tone="slate">{tipo.replaceAll("_", " ")}</Badge>)}
          </div>
        </Card>
        <Card className="p-4">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Cliente e ramo</p>
          <p className="mt-2 font-medium text-slate-900 dark:text-white">{dossie.identificacao?.cliente?.nome || "Cliente não identificado"}</p>
        ...
        <Card className="p-4">
          ...
          <p className="mt-2 text-lg font-semibold text-slate-900 dark:text-white">
            {typeof honorario === "number" ? honorario.toLocaleString("pt-BR", { style: "currency", currency: "BRL" }) : "Sem base OAB suficiente"}
          </p>
        ...
        <Card className="p-4">
          ...
          <p className="mt-2 text-lg font-semibold text-slate-900 dark:text-white">
            {typeof estimativa?.percentual === "number" ? `${estimativa.percentual}%` : "Sem base verificável"}
          </p>
      ...
      {estimativa?.base_estimativa && (
        <Alert variant="info" title="Base da estimativa">{estimativa.base_estimativa}</Alert>
      )}

      <Secao title="Cliente, partes e identificação" open>
      ...
      <Secao title="Fatos e cronologia" open>
        <p className="whitespace-pre-wrap text-sm leading-relaxed text-slate-700 dark:text-slate-200">{dossie.fatos?.sumario || "Síntese ainda não disponível."}</p>
        ...
        {(dossie.fatos?.cronologia ?? []).length > 0 && (
          <div className="mt-4"><p className="mb-2 text-xs font-semibold uppercase text-slate-400">Cronologia registrada</p><Lista itens={(dossie.fatos.cronologia as AnyRecord[]).map((e) => `${e.data || "data a confirmar"} — ${e.evento}`)} /></div>
        )}
      </Secao>

      <Secao title="Provas e matriz Fato × Prova × Tese" open>
        <div className="space-y-3">
          {matriz.length === 0 ? <p className="text-sm text-slate-500">A análise ainda não produziu correlações suficientes.</p> : matriz.map((linha, index) => (
            <Card key={index} className="p-3">
              <p className="text-sm"><strong>Fato:</strong> {rotulo(linha.fato)}</p>
              <p className="mt-1 text-sm"><strong>Prova:</strong> {rotulo(linha.prova)} {linha.prova_ja_disponivel ? <Badge tone="green">disponível</Badge> : <Badge tone="amber">a obter</Badge>}</p>
      ...
      <Secao title="Lacunas e perguntas ao advogado">
        {perguntas.length === 0 ? <Alert variant="success">Nenhuma lacuna estruturada adicional foi identificada nesta rodada.</Alert> : (
          <div className="space-y-3">{perguntas.map((item, index) => (
            <Card key={index} className="p-3"><p className="text-sm font-medium text-slate-900 dark:text-white">{item.pergunta}</p><p className="mt-1 text-xs text-slate-500">{item.motivo}</p></Card>
          ))}</div>
        )}
      </Secao>

      <Secao title="Teses, fundamentos e riscos">
        ...
        <p className="mb-2 mt-4 text-xs font-semibold uppercase text-slate-400">Teses do banco do escritório</p>
      ...
      <Secao title="Contradições e leitura adversarial">
        <Alert variant="warning" title="Hipóteses para revisão">{dossie.contradicoes_e_adversarial?.observacao}</Alert>
        <div className="mt-3"><Lista itens={dossie.contradicoes_e_adversarial?.falhas_da_parte_contraria} /></div>
        {dossie.contradicoes_e_adversarial?.critica_adversarial?.relatorio && (
          <pre className="mt-4 whitespace-pre-wrap rounded-lg bg-slate-50 p-3 text-sm text-slate-700 dark:bg-slate-950 dark:text-slate-200">{dossie.contradicoes_e_adversarial.critica_adversarial.relatorio}</pre>
        )}
      </Secao>

      <Secao title="Honorários sugeridos">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-3">
          {(["minimo_etico", "recomendado", "estrategico"] as const).map((faixa) => {
            ...
            return <Card key={faixa} className="p-3"><p className="text-xs font-semibold uppercase text-slate-400">{faixa.replaceAll("_", " ")}</p><p className="mt-1 font-semibold">{typeof item?.valor === "number" ? item.valor.toLocaleString("pt-BR", { style: "currency", currency: "BRL" }) : "—"}</p><p className="mt-1 text-xs text-slate-500">{item?.memoria_calculo}</p></Card>;
          })}
        </div>
      </Secao>

      <Secao title="Plano jurídico" open>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {!aprovado ? (
            <Button variant="primary" onClick={aprovarPlano} disabled={aprovando} icon={<CheckCircle2 className="h-4 w-4" />}>
          ...
          {aprovado && (
            <Button variant="ai" onClick={prepararPeca} disabled={preparandoPeca} icon={<Gavel className="h-4 w-4" />}>
              {preparandoPeca ? "Preparando…" : "Gerar peça"}
            </Button>
          )}
        </div>
      </Secao>
      ...
      {aprovado && planoPeca && (
        <Card className="space-y-4 border-ai-200 p-5">
          <div className="flex items-center gap-2"><Scale className="h-5 w-5 text-ai-600" /><h3 className="font-semibold">Motor de Peça</h3></div>
          ...
          {!planoPeca.checklist?.pronto && (
            <Alert variant="warning" title="Checklist bloqueante pendente"><Lista itens={checklistPeca.filter((i) => !i.ok && !i.presente)} /></Alert>
          )}
          <div className="grid gap-3 md:grid-cols-[220px_1fr] md:items-end">
            <label className="text-sm font-medium text-slate-700 dark:text-slate-200">
              Termo inicial do prazo
              <input type="date" value={termoInicial} onChange={(e) => { setTermoInicial(e.target.value); setTermoConfirmado(false); }} className="mt-1 block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 dark:border-slate-600 dark:bg-slate-900" />
            </label>
            <label className="flex items-center gap-2 text-sm text-slate-700 dark:text-slate-200">
              <input type="checkbox" checked={termoConfirmado} onChange={(e) => setTermoConfirmado(e.target.checked)} />
              Confirmo que revisei o termo inicial informado
            </label>
          </div>
          {gatePeca && <Alert variant="warning" title="Geração bloqueada"><AlertTriangle className="mr-1 inline h-4 w-4" />{gatePeca}</Alert>}
          <Button variant="primary" onClick={gerarPeca} disabled={gerandoPeca || !planoPeca.checklist?.pronto} icon={<FileText className="h-4 w-4" />}>
            {gerandoPeca ? "Gerando rascunho…" : "Gerar rascunho da peça"}
          </Button>
        </Card>
      )}
      ...
      {resultadoPeca && (
        <Alert variant="success" title="Peça gerada para revisão">
          O Motor de Peça concluiu a geração como rascunho. Prazo confirmado: {resultadoPeca.deadline?.data_prazo || "—"}. A revisão humana e os gates de citação continuam obrigatórios.
        </Alert>
      )}
    </div>
  );
