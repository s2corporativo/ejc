...
export function Confirmacao({
  ...
  erro409: string | null;
  onCriar: () => void;
  onVincularCasoExistente: (caseId: string) => void;
  onDescartar: () => void;
}) {
            ...
            <div className="min-w-0 flex-1 space-y-3">
              ...
              <div>
                ...
                {!cnjValido && (
                  <p className="mt-1 text-xs font-medium text-red-600">
                    Informe o CNJ no formato 0000000-00.0000.0.00.0000.
                  </p>
                )}
              </div>
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <span>
                          ...
                          {item.excluido ? " — excluído" : ""}
                        </span>
                        <div className="flex flex-wrap gap-2">
                          {item.caseId && !item.excluido && (
                            <Link
                          ...
                          {item.caseId &&
                            ...
                            !item.excluido &&
                            EH_CNJ.test(proposta.numeroCnj) && (
                              <Button
                                type="button"
                                variant="secondary"
                                size="sm"
                                disabled={criando}
                                onClick={() =>
                                  onVincularCasoExistente(item.caseId as string)
                                }
                              >
                                Vincular CNJ a este caso
                              </Button>
                            )}
                        </div>
                      </div>
              ...
              {bloqueioProcessual && (
                <p className="text-sm font-semibold text-red-700 dark:text-red-300">
                  Outro caso não será criado. Abra ou restaure o registro
                  existente.
                </p>
              )}
            </div>
