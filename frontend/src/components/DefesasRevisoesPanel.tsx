...
const arraySeguro = (valor: unknown): any[] =>
...
function Lista({ titulo, itens }: { titulo: string; itens?: unknown }) {
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-white/10 dark:bg-white/[0.03]">
      <h4 className="mb-3 text-sm font-semibold text-slate-950 dark:text-slate-50">
        {titulo}
      </h4>
      <div className="space-y-2">
        {lista.map((item: any, index) => (
          <div
            key={index}
            className="rounded-xl bg-slate-50 p-3 text-sm dark:bg-white/[0.04]"
          >
            {typeof item === "string" ? (
              ...
              <>
                <div className="flex flex-wrap items-start justify-between gap-2">
                  ...
                  {item?.risco && (
                    <span
                      className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ring-1 ${riscoClasse(item.risco)}`}
                    >
                      {item.risco}
                    </span>
                  )}
                ...
                {(item?.origem_literal || item?.trecho_literal) && (
                ...
                {arraySeguro(item?.fatos).length > 0 && (
                  <p className="mt-1 text-xs text-slate-500">
                    Fatos: {arraySeguro(item.fatos).join("; ")}
                  </p>
                )}
                {arraySeguro(item?.provas).length > 0 && (
                  <p className="mt-1 text-xs text-slate-500">
                    Provas: {arraySeguro(item.provas).join("; ")}
                  </p>
                )}
              </>
            )}
...
export default function DefesasRevisoesPanel() {
  ...
  const [resultadoEntrada, setResultadoEntrada] =
  ...
  useEffect(() => {
    Promise.all([
      ...
      .catch(() =>
        toast.error(
          "Não foi possível carregar o módulo de Defesas e Revisões.",
        ),
      );
  }, []);
  ...
  const resumoParaMotor = () => {
    ...
    const teses = arraySeguro(resultado.teses || resultado.matriz_vicios_teses)
      ...
      .map(
        (item: any) =>
          `- ${item?.titulo || item?.vicio_ou_tese || "Tese"}: ${item?.fundamento || "verificar"}`,
      )
      .join("\n");
    const pendencias = arraySeguro(resultado.documentos_faltantes)
      .map((item) => `- ${item}`)
      .join("\n");
    return [
  ...
  const salvarNoCaso = async () => {
    ...
    try {
      ...
      if (resultadoEntrada?.batch_id) {
        await api.post(
          `/entrada-universal/${resultadoEntrada.batch_id}/preparar-pacote`,
          { salvar_no_caso: true },
        );
      }
  ...
  const encaminharMotor = async () => {
    if (!podeMotor)
      return toast.error(
        "A geração da peça exige advogado ou gestor jurídico.",
      );
    if (!caseId) return toast.error("Vincule a análise a um caso.");
    const codigo = resultado?.motor_peca_codigo;
    if (!codigo)
      return toast.error(
        "Resolva as pendências impeditivas ou use o gerador geral de peças.",
      );
  ...
  return (
    <section
      id="defesas-revisoes"
      className="space-y-4 rounded-xl border border-primary-100 bg-white p-4 dark:border-white/10 dark:bg-white/[0.03]"
    >
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-start">
        <div className="max-w-3xl">
          <div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-primary-700">
            <ShieldCheck className="h-4 w-4" /> Módulo jurídico integrado
          </div>
          <h2 className="text-2xl font-semibold text-slate-950 dark:text-white">
            Defesas e Revisões
          </h2>
          <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-300">
            Importe o pacote completo, organize documentos, confira a prontidão
            e produza a estratégia para trânsito, ambiental, administrativo,
            contratos e bancos.
          </p>
        </div>
        <div className="rounded-2xl bg-warn-50 p-3 text-xs text-warn-800 ring-1 ring-warn-200">
          <div className="flex gap-2">
            <AlertTriangle className="mt-0.5 h-4 w-4 shrink-0" />
            <span>
              Prazos, abusividade, classificação e cabimento nunca são
        ...
        {modalidades.map((item) => {
          ...
          return (
            <button
              ...
              type="button"
              onClick={() => {
                setModalidade(item.codigo);
                setResultado(null);
                setResultadoEntrada(null);
              }}
              className={`rounded-2xl border p-3 text-left transition ${ativo ? "border-primary-400 bg-primary-50 ring-2 ring-primary-100 dark:bg-primary-950/30" : "border-slate-200 bg-white hover:-translate-y-0.5 hover:border-primary-200 dark:border-white/10 dark:bg-white/[0.03]"}`}
            >
              <Icon
                className={`mb-2 h-5 w-5 ${ativo ? "text-primary-700" : "text-slate-500"}`}
              />
      ...
      {analisando && (
        <div className="flex items-center justify-center gap-2 rounded-2xl border border-primary-100 bg-primary-50 p-5 text-sm font-medium text-primary-800">
          <Loader2 className="h-5 w-5 animate-spin" /> Aplicando o motor
          jurídico especializado ao pacote documental…
        </div>
      )}
        ...
        <div className="space-y-4 border-t border-slate-200 pt-5 dark:border-white/10">
          <div className="grid gap-3 lg:grid-cols-3">
            <div className="rounded-2xl bg-slate-950 p-4 text-white lg:col-span-2">
              ...
              <h3 className="mt-1 text-lg font-semibold">
                {resultado.peca_recomendada?.nome || "Providência a confirmar"}
              </h3>
            ...
            <div className="rounded-2xl bg-warn-50 p-4 text-warn-900 ring-1 ring-warn-200">
              ...
              <p className="mt-2 text-sm font-medium">
                {resultado.prazo?.data_expressa_no_documento ||
                  resultado.prazo?.data_expressa ||
                  "Sem data final confirmada"}
              </p>
              <p className="mt-1 text-xs leading-5">
                {resultado.prazo?.regra || "Verificar regra e termo inicial."}
              </p>
          ...
          {resultado.juros_taxas && modalidade === "revisao_bancaria" && (
            <section className="rounded-2xl border border-primary-200 bg-primary-50 p-4 dark:bg-primary-950/20">
              <h4 className="flex items-center gap-2 text-sm font-semibold text-primary-950 dark:text-primary-100">
                <Banknote className="h-4 w-4" /> Juros, CET e encargos
                identificados
              </h4>
              <div className="mt-3 grid gap-2 text-sm sm:grid-cols-2 lg:grid-cols-4">
                <div>
                  ...
                  <b>
                    {resultado.juros_taxas.taxa_mensal ?? "não identificada"}
                  </b>
                </div>
                <div>
                  <span className="block text-xs text-slate-500">
                  ...
                  <b>
                    {resultado.juros_taxas.taxa_anual ?? "não identificada"}
                  </b>
                </div>
                <div>
                  <span className="block text-xs text-slate-500">CET</span>
                  <b>{resultado.juros_taxas.cet ?? "não identificado"}</b>
                </div>
                <div>
                  <span className="block text-xs text-slate-500">
                    Capitalização
                  </span>
                  <b>{resultado.juros_taxas.capitalizacao ?? "indefinido"}</b>
                </div>
              </div>
            </section>
          )}
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <Lista
              titulo="Datas e eventos encontrados"
              itens={resultado.datas_eventos}
            />
            <Lista titulo="Vícios formais" itens={resultado.vicios_formais} />
            <Lista
              titulo="Questões de mérito"
              itens={resultado.questoes_de_merito}
            />
            <Lista
              titulo="Matriz de teses"
              itens={resultado.teses || resultado.matriz_vicios_teses}
            />
            <Lista
              titulo="Checklist obrigatório"
              itens={resultado.checklist_obrigatorio}
            />
            <Lista
              titulo="Documentos faltantes"
              itens={resultado.documentos_faltantes}
            />
          </div>
          ...
          {resultado.motor_peca && (
            <section className="rounded-2xl border border-success-200 bg-success-50 p-4">
              <h4 className="text-sm font-semibold text-success-900">
                Motor de Peça concluído
              </h4>
              <p className="mt-1 text-sm text-success-800">
                Peça principal:{" "}
                {resultado.motor_peca.peca_principal || "a confirmar"}.
                Checklist:{" "}
                {resultado.motor_peca.checklist?.pronto
                  ? "pronto"
                  : "possui pendências"}
                .
              </p>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              ...
              Persistir jornada no caso
            </button>
            {resultado.motor_peca_codigo && podeMotor && (
              <button
                className="btn-gold flex items-center gap-2"
                onClick={encaminharMotor}
                disabled={encaminhando || !caseId}
              >
                {encaminhando ? (
                  <Loader2 className="h-4 w-4 animate-spin" />
                ) : (
                ...
                Encaminhar ao Motor de Peça
              </button>
            )}
            {!resultado.motor_peca_codigo && podeMotor && (
              <Link
                to={`/pecas${caseId ? `?case_id=${caseId}` : ""}`}
                className="btn-gold flex items-center gap-2"
              >
                <ArrowRight className="h-4 w-4" /> Abrir gerador de peças
              </Link>
            )}
            {caseId && (
          ...
          <DefesasRevisoesComplementos
            modalidade={modalidade}
            caseId={caseId}
            resultado={resultado}
          />

          {arraySeguro(resultado.fontes).length > 0 && (
            <section className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-white/10 dark:bg-white/[0.03]">
              <h4 className="text-sm font-semibold text-slate-900 dark:text-white">
                Fontes recuperadas pela IA
              </h4>
              <ul className="mt-2 space-y-1 text-xs text-slate-500">
                {arraySeguro(resultado.fontes).map(
                  (fonte: any, index: number) => (
                    <li key={index}>
                      • {fonte?.titulo || fonte?.fonte || "Fonte interna"}
