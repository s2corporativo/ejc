...
type Aba =
  "operacao" | "comparar" | "calculos" | "viabilidade" | "decisao" | "memoria";
...
export default function DefesasRevisoesComplementos({
  modalidade,
  caseId,
  resultado,
}: Props) {
  ...
  const persistir = () => {
    if (!caseId)
      return toast.error("Selecione um caso para persistir o diagnóstico.");
    return executar(
      "persistir",
      async () =>
        (
  ...
  const pacote = () => {
    ...
    return executar(
      ...
      async () =>
        (
          await api.post("/defesas-revisoes/avancado/pacote", {
            case_id: caseId,
            modalidade,
            resultado,
          })
  ...
  const adversarial = () =>
    executar(
      ...
      async () =>
        (
          await api.post("/defesas-revisoes/avancado/adversarial", {
  ...
  const comparar = () => {
    if (!base || !comparado)
      return toast.error("Selecione os dois documentos para comparação.");
    ...
    return executar(
      "comparar",
      async () =>
        (await api.post("/defesas-revisoes/avancado/comparar-documentos", form))
          .data,
  ...
  const calcular = () => {
    ...
    return executar(
      ...
      async () =>
        (
          await api.post(
            "/defesas-revisoes/avancado/calcular-especialidade",
            payload,
          )
        ).data,
  ...
  const viabilidade = () => {
    ...
    [
      ...
      "outras_despesas",
      "risco_sucumbencia",
      "probabilidade_exito_pct",
    ].forEach((chave) => {
      payload[chave] = numero(valores[chave] || "0");
    });
    return executar(
      "viabilidade",
      async () =>
        (await api.post("/defesas-revisoes/avancado/viabilidade", payload))
          .data,
  ...
  const analisarDecisao = () => {
    if (!caseId || !decisao)
      return toast.error("Selecione o caso e a nova decisão.");
    const form = new FormData();
    form.append("modalidade", modalidade);
    ...
    return executar(
      "decisao",
      async () =>
        (await api.post("/defesas-revisoes/avancado/analisar-decisao", form))
          .data,
  ...
  const memoria = () =>
    executar(
      "memoria",
      async () =>
        (await api.get(`/defesas-revisoes/avancado/memoria/${modalidade}`))
          .data,
  ...
  const campo = (chave: string, label: string, placeholder = "0") => (
    <label className="block">
      ...
      <input
        ...
        value={valores[chave] || ""}
        onChange={(event) =>
          setValores((atual) => ({ ...atual, [chave]: event.target.value }))
        }
    ...
    if (modalidade === "multa_transito")
      return (
        <div className="grid gap-3 md:grid-cols-2">
          ...
          {campo(
            "limite_informado",
            "Limite aplicável confirmado",
            "Informe somente após conferir",
          )}
        </div>
      );
    if (
      modalidade === "multa_ambiental" ||
      modalidade === "multa_administrativa"
    )
      return (
    ...
    <section className="space-y-4 rounded-xl border border-slate-200 bg-white p-4 dark:border-white/10 dark:bg-white/[0.03]">
      <div>
        ...
        <h3 className="mt-1 text-lg font-semibold text-slate-950 dark:text-white">
          Concluir, validar e acompanhar o caso
        </h3>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-300">
          Persista a estratégia, gere o pacote, confronte a tese, compare
          documentos, faça cálculos e analise a próxima decisão.
        </p>
      </div>
      ...
      {aba === "operacao" && (
        <div className="grid gap-3 md:grid-cols-3">
          <button
            ...
            onClick={persistir}
            disabled={!caseId || Boolean(loading)}
          >
            {loading === "persistir" ? (
      ...
      {aba === "comparar" && (
        <div className="space-y-3">
          <div className="grid gap-3 md:grid-cols-2">
            <label>
              <span className="label">Documento-base</span>
              <input
                className="input w-full"
                type="file"
            ...
            <label>
              <span className="label">Documento novo/aditivo</span>
              <input
                className="input w-full"
                type="file"
          ...
          <button
            ...
            disabled={Boolean(loading)}
          >
            <FileDiff className="h-4 w-4" /> Comparar integralmente
          </button>
        </div>
      )}
      ...
      {aba === "viabilidade" && (
        <div className="space-y-3">
          ...
          <button
            ...
            disabled={Boolean(loading)}
          >
            <Scale className="h-4 w-4" /> Avaliar viabilidade
          </button>
        </div>
      )}
      ...
      {aba === "decisao" && (
        <div className="space-y-3">
          <label>
            <span className="label">Nova decisão, julgamento ou resposta</span>
            <input
              className="input w-full"
              type="file"
              onChange={(e) => setDecisao(e.target.files?.[0] || null)}
            />
          </label>
          <button
            className="btn-gold flex items-center gap-2"
            onClick={analisarDecisao}
            disabled={!caseId || Boolean(loading)}
          >
            <FileSearch className="h-4 w-4" /> Comparar e indicar próxima medida
          </button>
        </div>
      )}
      ...
      {aba === "memoria" && (
        <button
          ...
          disabled={Boolean(loading)}
        >
          <Archive className="h-4 w-4" /> Consultar casos semelhantes do
          escritório
        </button>
      )}

      {loading && !["persistir", "pacote", "adversarial"].includes(loading) && (
        <div className="flex items-center gap-2 rounded-xl bg-primary-50 p-3 text-sm text-primary-800">
