...
function Regras({ regras }: { regras: readonly RegraGuiaTributario[] }) {
  return (
    <div className="space-y-2">
      {regras.map((item) => (
        <div key={item.tema} className="rounded-lg border border-slate-100 bg-slate-50/50 p-3">
...
export default function GuiaTributario() {
  ...
  const feitos = CHECKLIST_TRIBUTARIO.filter((_, index) => marcados[index]).length;
  ...
  return (
    <div className="card mb-4 border-l-4 border-warn-500 p-4">
      ...
      <p className="mb-3 text-xs leading-5 text-slate-500">
        Instrumento interno de apoio. O guia não calcula prazo fatal, não substitui
        a leitura do ato de ciência e não transforma hipótese tributária em crédito
        recuperável. Toda conclusão deve registrar fonte oficial, vigência, ente,
        marco inicial e revisão profissional.
      </p>
      ...
      <div className="space-y-2">
        <Sec icon={ListChecks} titulo="1. Checklist de saneamento do caso" aberto>
          <div className="mb-2 flex items-center justify-between text-[11px] text-slate-500">
            <span>Marque somente o que foi efetivamente conferido no caso.</span>
            ...
            {CHECKLIST_TRIBUTARIO.map((item, index) => (
              ...
              >
                <input
                  ...
                  className="mt-0.5"
                />
                <span className={marcados[index] ? "text-slate-400 line-through" : ""}>
                  {item}
                </span>
              </label>
            ))}
        ...
        <Sec icon={Clock} titulo="2. PAF Federal — prazos e transição de 2026" aberto>
          ...
          <div className="rounded-lg border border-slate-200 bg-white p-3 text-[11px] leading-5 text-slate-600">
            ...
            vigente → impugnação/DRJ → decisão → verificar recurso cabível e prazo
            vigente → CARF/CSRF conforme admissibilidade. Não usar prazo do recurso
            especial sem consultar o RICARF vigente.
          </div>
        </Sec>
        ...
        <Sec icon={Scale} titulo="4. CARF — voto de qualidade e recursos">
          ...
          <p className="rounded-lg border border-warn-200 bg-warn-50 p-3 text-[11px] leading-5 text-warn-900">
            ...
            ao contribuinte. Essa formulação foi removida por ser incompatível com
            a Lei 14.689/2023. Para recurso especial, competência, admissibilidade e
            rito, consultar o RICARF vigente na data do ato.
          </p>
        </Sec>
        ...
        <Sec icon={FileText} titulo="7. Judicialização, execução fiscal e repetição">
          ...
          <p className="rounded-lg border border-slate-200 bg-white p-3 text-[11px] leading-5 text-slate-600">
            ...
            indébito têm pressupostos e termos iniciais próprios. O encerramento do
            processo administrativo é um fato relevante, mas não gera
