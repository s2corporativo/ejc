  ...
  return (
    <div className="space-y-5">
                    ...
                    <div className="mt-2 flex items-center gap-1 text-[11px]">
                      {effective ? (
                        <>
                          ...
                          <span className="text-success-700">Efetivo</span>
                        </>
                      ) : (
                        <span className="text-slate-400">
                          {available ? "Desativado pelo usuário" : "Indisponível no servidor"}
                        </span>
                      )}
                    </div>
                  </div>
      ...
      <SectionCard
        ...
        subtitle="O horário silencioso vale para canais externos; notificações internas continuam registradas."
      >
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-slate-500">
                  Preferência armazenada para o digest diário quando o job estiver habilitado.
                </div>
              </div>
            </div>
            <Toggle
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <Toggle
                ...
                onChange={(checked) => {
                  patch("quiet_hours_start", checked ? "22:00:00" : null);
                  patch("quiet_hours_end", checked ? "07:00:00" : null);
                }}
              />
            </div>
            {quietEnabled && (
              <div className="mt-4 grid max-w-md grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      patch("quiet_hours_start", `${event.target.value}:00`)
                    }
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      patch("quiet_hours_end", `${event.target.value}:00`)
                    }
                  />
                </div>
              </div>
      ...
      <SectionCard
        ...
        subtitle={devices?.notice || "Inscrições de navegador sem exposição das chaves push."}
      >
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <div className="text-sm text-slate-500">
            {devices?.total ?? 0} dispositivo(s) inscrito(s)
          </div>
          <div className="flex gap-2">
            ...
            <button
              ...
              disabled={!data.available_channels.push || busyDevice === "activate"}
        ...
        <div className="divide-y divide-slate-100 overflow-hidden rounded-xl border border-slate-200 bg-white">
          {!devices?.data.length ? (
            <div className="p-6 text-center text-sm text-slate-400">
              Nenhum dispositivo push inscrito.
            </div>
          ) : (
      ...
      <div className="sticky bottom-4 flex justify-end gap-2 rounded-xl border border-slate-200 bg-white/95 p-3 shadow-lg backdrop-blur">
        <button
          type="button"
          className="btn-secondary"
          disabled={!hasChanges || saving}
          onClick={() => setForm(data.preferences)}
        >
          Descartar
        </button>
        <button
