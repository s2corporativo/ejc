...
const FocusToday: React.FC = () => {
    ...
    if (!data) return <div className="text-amber-500 animate-pulse">Sincronizando com o Cérebro EJC...</div>;

    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6">
            {/* Coluna 1: Crítico */}
            <div className="bg-stone-900 border-l-4 border-red-700 p-6 rounded shadow-lg">
                ...
                <div className="space-y-3">
                    {data.next_deadlines.map((item, i) => (
                        <div key={i} className="flex justify-between items-center bg-stone-800 p-3 rounded border border-stone-700">
                            ...
                            <span className="text-xs bg-red-900/30 text-red-400 px-2 py-1 rounded">URGENTE</span>
                        </div>
                    ))}
                </div>
            </div>
            ...
            <div className="bg-stone-900 border-l-4 border-amber-600 p-6 rounded shadow-lg">
                ...
                <div className="space-y-3">
                    {data.revenue_opportunities.map((item, i) => (
                        <div key={i} className="bg-amber-900/10 p-3 rounded border border-amber-900/20">
                            <p className="text-amber-200 text-sm mb-2">{item}</p>
                            <button className="text-xs bg-amber-600 hover:bg-amber-500 text-white px-3 py-1 rounded transition-colors">
                                ACIONAR COBRANÇA
                            </button>
                        </div>
                    ))}
