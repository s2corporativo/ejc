...
const RadarLegislativo: React.FC = () => {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-6 bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl shadow-2xl border border-amber-500/20">
      {/* Header */}
      <div className="mb-8">
      ...
      {filteredAlertas.length > 0 && (
        <div className="mb-8 bg-red-900/20 border border-red-500/30 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-4">
            <AlertCircle className="w-6 h-6 text-red-500" />
            <h3 className="text-xl font-bold text-red-400">Alertas Regulatórios</h3>
          </div>
          <div className="space-y-3">
            {filteredAlertas.map(alerta => (
              <div
                key={alerta.id}
                className="bg-slate-800/50 border border-red-500/20 rounded-lg p-4 hover:border-red-500/50 transition-all"
              >
                <div className="flex items-start justify-between">
                  ...
                  {!alerta.resolvido && (
                    <div className="w-3 h-3 bg-red-500 rounded-full animate-pulse ml-4 mt-1" />
                  )}
                </div>
              </div>
            ))}
        ...
        <div className="space-y-3">
          {loading ? (
            ...
            filteredLegislacoes.map(leg => (
              <div
                key={leg.id}
                className="bg-slate-800/50 border border-amber-500/20 rounded-lg p-4 hover:border-amber-500/50 transition-all hover:shadow-lg hover:shadow-amber-500/10"
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                  ...
                  <a
                    ...
                    className="ml-4 px-3 py-1 bg-amber-500 text-slate-900 rounded font-medium text-sm hover:bg-amber-400 transition-all"
                  >
                    Acessar
                  </a>
                </div>
