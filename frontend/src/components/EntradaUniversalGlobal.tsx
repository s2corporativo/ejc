          ...
          <div className="w-full max-w-6xl rounded-xl border border-slate-200 bg-canvas shadow-float dark:border-white/10">
            ...
            <div className="p-4 md:p-5">
              <div className="mb-4 max-w-xl">
                ...
                >
                  {MODALIDADES.map((item) => (
                    <option key={item.value || "geral"} value={item.value}>
                      {item.label}
                    </option>
                  ))}
                </select>
              ...
              <EntradaUniversalDocumentos
                ...
                titulo={
                  caseId
                    ? "Adicionar pacote documental ao caso"
                    : "Importar pacote documental"
                }
              ...
              {ultimoLote && (
                <div className="mt-4 rounded-xl border border-primary-200 bg-primary-50 p-3 text-xs text-primary-900">
                  Lote <b>{ultimoLote.batch_id}</b> concluído.{" "}
                  {caseId
                    ? "Os documentos já pertencem ao caso atual."
                    : "O lote permanece auditável no GED e pode ser vinculado posteriormente a um caso pelo fluxo jurídico."}
                </div>
              )}
            </div>
          </div>
