...
const AREAS = ['Administrativa', 'Tributaria', 'Trabalhista', 'Ambiental', 'Bancaria'];
...
export const VictoryVaultPanel: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const params = { query: searchQuery || undefined, area_juridica: selectedArea || undefined };
    ...
    <div className="space-y-4">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {teses.length === 0 ? (
            <div className="py-10 text-center text-sm text-slate-400">Nenhuma tese encontrada</div>
          ) : (
            teses.map((tese) => (
              <div key={tese.id} className="card p-4">
                <div className="flex items-start justify-between gap-3">
                  <h3 className="text-sm font-semibold text-slate-900">{tese.titulo}</h3>
                  <span className="badge badge-info shrink-0">{tese.area_juridica}</span>
                </div>
                <p className="mt-1 text-sm text-slate-600">{tese.ementa}</p>
                {tese.processo_referencia && (
                  <p className="mt-2 text-xs text-slate-400">Processo: {tese.processo_referencia}</p>
                )}
                <p className="mt-2 text-xs text-slate-400">
                  Vitoria em {new Date(tese.data_vitoria).toLocaleDateString('pt-BR')}
                </p>
              </div>
            ))
          )}
        ...
        <div className="space-y-3">
          {modelos.length === 0 ? (
            <div className="py-10 text-center text-sm text-slate-400">Nenhum modelo encontrado</div>
          ) : (
            modelos.map((modelo) => (
              <div key={modelo.id} className="card p-4">
                <div className="flex items-start justify-between gap-3">
                  <h3 className="text-sm font-semibold text-slate-900">{modelo.descricao || modelo.tipo_documento}</h3>
                  <span className="badge badge-neutral shrink-0">{modelo.tipo_documento}</span>
                </div>
                <p className="mt-1 text-xs text-slate-500">Area: {modelo.area_juridica}</p>
                <button className="btn-outline mt-3 text-xs">Usar modelo</button>
              </div>
            ))
          )}
        </div>
