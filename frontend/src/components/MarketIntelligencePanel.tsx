    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-stone-950 p-8">
            <div className="max-w-7xl mx-auto">
                {/* Header */}
                <div className="mb-12">
                    <h1 className="text-4xl font-serif font-bold text-warn-500 mb-2">📊 Market Intelligence</h1>
                ...
                {activeTab === 'competitors' && (
                    <div className="space-y-6">
                        <div className="rounded-2xl bg-gradient-to-br from-stone-800/50 to-stone-900/50 border border-warn-900/20 p-8 backdrop-blur-xl">
                            <h2 className="text-2xl font-serif font-bold text-warn-400 mb-6">Principais Concorrentes (Betim/MG)</h2>
                            <div className="space-y-4">
                                {[
                                    ...
                                    <div key={i} className="p-6 bg-stone-700/30 rounded-lg border border-stone-700 hover:border-warn-600/40 transition-all">
                                        <div className="flex justify-between items-start mb-4">
                                            <div>
                                                <h3 className="text-lg font-bold text-white">{competitor.name}</h3>
                                                <p className="text-sm text-stone-400">Licitações Vencidas: <span className="text-warn-400 font-bold">{competitor.wins}</span></p>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-xs text-stone-400">Fator de Preço</p>
                                                <p className={`text-lg font-bold ${competitor.price_factor < 1 ? 'text-green-400' : 'text-danger-400'}`}>
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {competitor.specialties.map((spec, j) => (
                                                <span key={j} className="text-xs bg-warn-900/20 text-warn-300 px-3 py-1 rounded-full">
                                                    {spec}
                                                </span>
                                            ))}
                                        </div>
                ...
                {activeTab === 'pricing' && (
                    <div className="rounded-2xl bg-gradient-to-br from-stone-800/50 to-stone-900/50 border border-warn-900/20 p-8 backdrop-blur-xl">
                        <h2 className="text-2xl font-serif font-bold text-warn-400 mb-6">Análise de Preços</h2>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div className="p-6 bg-stone-700/20 rounded-lg border border-stone-700">
                                ...
                                <p className="text-3xl font-serif font-bold text-warn-400">R$ 10.000</p>
                            </div>
                            <div className="p-6 bg-stone-700/20 rounded-lg border border-stone-700">
                            ...
                            <div className="p-6 bg-stone-700/20 rounded-lg border border-stone-700">
                                ...
                                <p className="text-3xl font-serif font-bold text-warn-500">+15%</p>
                ...
                {activeTab === 'niches' && (
                    <div className="space-y-6">
                        <div className="rounded-2xl bg-gradient-to-br from-stone-800/50 to-stone-900/50 border border-warn-900/20 p-8 backdrop-blur-xl">
                            <h2 className="text-2xl font-serif font-bold text-warn-400 mb-6">Recomendações de Nicho</h2>
                            <div className="space-y-4">
                                {[
                                    ...
                                    <div key={i} className={`p-6 rounded-lg border ${rec.roi === 'Alto' ? 'bg-green-900/10 border-green-600/30' : 'bg-warn-900/10 border-warn-600/30'}`}>
                                        <div className="flex justify-between items-start mb-3">
                                            <h3 className="text-lg font-bold text-white">{rec.niche}</h3>
                                            <span className={`px-3 py-1 rounded-full text-xs font-bold ${rec.roi === 'Alto' ? 'bg-green-900/30 text-green-300' : 'bg-warn-900/30 text-warn-300'}`}>
                                                ROI: {rec.roi}
                                            </span>
                                        </div>
