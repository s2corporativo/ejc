...
const EscritaAssistida: React.FC = () => {
    ...
    return (
        <div className="bg-stone-900 border border-warn-900/30 p-8 rounded-lg shadow-2xl max-w-4xl mx-auto">
            <h2 className="text-3xl font-serif text-warn-500 mb-6">Escrita Assistida (Modo Elite)</h2>
            
            <div className="flex justify-between mb-8">
                {[1, 2, 3].map((s) => (
                    <div key={s} className={`flex items-center ${step >= s ? 'text-warn-500' : 'text-stone-600'}`}>
                        <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center mr-2 ${step >= s ? 'border-warn-500' : 'border-stone-600'}`}>
                            {s}
                        </div>
            ...
            {step === 2 && (
                <div className="space-y-4 animate-fade-in">
                    <div className="bg-warn-900/10 border-l-4 border-warn-600 p-4">
                        <h3 className="text-warn-500 font-bold mb-2">Tese Sugerida pela IA:</h3>
            ...
            {step === 3 && (
                <div className="space-y-4 animate-fade-in">
                    <h3 className="text-warn-500 font-bold">Minuta Gerada (Visual Law Ready):</h3>
