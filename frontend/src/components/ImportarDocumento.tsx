...
export default function ImportarDocumento({
  ...
  const aplicar = (resultado: EntradaUniversalResultado) => {
    ...
    const tipoPrazo = ["administrativo", "ambiental", "transito"].includes(area)
      ? "administrativo"
      : "processual";
      ...
      ? [
          {
            titulo:
              prazo.evento ||
              prazo.providencia ||
              "Prazo identificado na importação",
            tipo: tipoPrazo,
    ...
    const patch: Patch = {
      ...
      prioridade:
        resultado.nivel_prontidao === "nao_apto_para_redacao"
          ? "alta"
          : "media",
      _cliente_candidato: {
        nome: pessoais.nome || partes.autor || undefined,
      ...
      _extracao: {
        origem: "entrada_universal",
        batch_id: resultado.batch_id,
        classificacao,
        identificacao_processual: identificacao,
        partes,
        ...
        matriz_vicios_teses: resultado.matriz_vicios_teses || [],
        datas_eventos: resultado.datas_eventos || [],
        prazo,
        prazos,
