...
// Tenta o fluxo visual de login/TOTP. Se a UI não exibir a segunda etapa,
// registra a falha, autentica pela mesma API com TOTP e continua a auditoria
// de todas as telas sem mascarar o defeito encontrado.
...
async function apiLogin(context) {
  const response = await context.request.post(`${BASE}/api/auth/login`, {
    data: { email: EMAIL, password: PASSWORD, totp_code: totp(TOTP_SECRET) },
    timeout: 30000,
  });
  const body = await response.json().catch(() => ({}));
  if (response.status() !== 200 || !body.access_token) {
    throw new Error(`Fallback de login falhou: HTTP ${response.status()} — ${JSON.stringify(body).slice(0, 800)}`);
  }
  return body;
}
...
async function main() {
  ...
  try {
    ...
    let visualTotp = false;
    try {
      await numeric.waitFor({ state: "visible", timeout: 5000 });
      visualTotp = true;
    } catch {
      visualTotp = false;
    }
    report.login.password_step_ms = Date.now() - t0;
    report.login.visual_totp_exibido = visualTotp;

    if (visualTotp) {
      await numeric.fill(totp(TOTP_SECRET));
      t0 = Date.now();
      await page.locator('button[type="submit"]').click();
      await page.waitForURL(url => !url.pathname.startsWith("/login"), { timeout: 30000 });
      await ready(page, 20000);
      report.login.totp_to_dashboard_ms = Date.now() - t0;
      report.login.metodo_conclusao = "fluxo_visual_completo";
    } else {
      const visibleText = await page.locator("body").innerText();
      report.login.erro_apos_senha = visibleText.slice(0, 2500);
      report.login.falha_fluxo_visual = "A segunda etapa TOTP não foi renderizada em até 5 segundos.";
      await shot(page, report, "01b-login-sem-totp-apos-senha");
      ...
      await ready(page, 20000);
      report.login.totp_to_dashboard_ms = Date.now() - t0;
      report.login.metodo_conclusao = "fallback_api_apos_falha_visual_registrada";
    }
    ...
    const globalInteractions = [
      ["busca_global", async () => { await page.keyboard.press("Control+K"); await page.waitForTimeout(150); await page.keyboard.press("Escape"); }],
    ...
    report.summary = {
      ...
      login_visual_failure: report.login.visual_totp_exibido === false,
    };
  } finally {
