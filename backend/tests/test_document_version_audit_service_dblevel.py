...
async def test_preflight_detecta_anomalias_sem_persistir_dados():
    ...
    try:
        async with Session() as db:
            ...
            # Raízes inválidas: documento declarado como raiz com versão ≠ 1
            # (raiz_invalida), documento raiz com versão 0 (invalido) e a raiz
            # do grupo cuja cadeia virou cíclica ganhou predecessor (ciclo_a).
            assert (
                depois.raizes_canonicas_invalidas
                == baseline.raizes_canonicas_invalidas + 3
            )
