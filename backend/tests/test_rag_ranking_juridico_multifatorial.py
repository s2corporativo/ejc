...
def test_score_autoridade_entra_como_sinal_secundario():
    alto, bonus_alto = reranker._enrich(_candidate(), "fraude bancária STJ consumidor")
    baixo_extra = dict(_candidate()["extra"])
    baixo_extra["score_autoridade"] = 10
    baixo, bonus_baixo = reranker._enrich(_candidate(extra=baixo_extra), "fraude bancária STJ consumidor")
    assert bonus_alto > bonus_baixo
    assert alto["governance_factors"]["score_autoridade"] > baixo["governance_factors"]["score_autoridade"]


def test_aderencia_de_tribunal_e_area_e_auditavel():
    item, _ = reranker._enrich(_candidate(), "responsabilidade por fraude bancária no STJ")
...
def test_norma_revogada_continua_penalizada():
    item, _ = reranker._enrich(
        _candidate(situacao_juridica={"code": "revogada", "label": "Revogada", "warning": True}),
        "fraude bancária",
    )
