"""Fluxo DJEN → revisão humana auditável de prazo — Issue #968.

Prova o contrato fail-safe:
- texto e disponibilização nunca fabricam vencimento;
- disponibilização, publicação, termo inicial e vencimento são distintos;
- regime processual é explícito;
- materialização exige confirmação humana da fonte oficial;
- cronologia mínima é validada;
- ``dias`` não é atalho para cálculo DJEN;
- aceite/recusa/tratamento permanecem auditáveis e idempotentes.
"""
...
class _FakeDB:
    """Fila de resultados para execute(); depois simula usuário jurídico ativo."""
    ...
    async def execute(self, *a, **k):
        ...
        # O router valida responsavel_id no banco antes de materializar o prazo.
        return _Res(_user())
...
def _payload(**kw):
    ...
    base = dict(
        data_publicacao=date(2026, 3, 3),
        termo_inicial=date(2026, 3, 4),
        regime_calculo="civel",
        data_prazo=date(2026, 4, 10),
        confirmacao_fonte_oficial=True,
    )
    base.update(kw)
    return intimacoes.AceitarPrazoRequest(**base)
...
def test_disponibilizacao_e_preservada_sem_fabricar_outros_marcos():
    from app.routers.intimacoes import _calcular_sugestao

    s = _calcular_sugestao(_com(data_disponibilizacao=date(2026, 3, 2)))
    assert s["data_disponibilizacao"] == date(2026, 3, 2)
    assert s["data_publicacao"] is None
    assert s["termo_inicial"] is None
    assert s["data_base"] is None
    ...
    s = _calcular_sugestao(
        _com(
            data_disponibilizacao=datetime(
                2026, 3, 2, 14, 30, tzinfo=timezone.utc
            )
        )
    )
    assert s["data_disponibilizacao"] == date(2026, 3, 2)
...
async def test_gestao_enxerga_intimacao_de_qualquer_advogado():
    ...
    out = await prazo_sugerido(
        com_id="com-1", db=db, cu=_user(UserRole.socio, "s1")
    )
...
async def test_aceitar_prazo_aplica_ownership_do_caso(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc:
        await intimacoes.aceitar_prazo(
            com_id="com-1", payload=_payload(), db=db, cu=_user()
        )
    assert exc.value.status_code == 403
    assert db.added == []


# ── Materialização auditável ─────────────────────────────────────────────────

@pytest.mark.anyio
async def test_revisao_incompleta_nao_cria_deadline(monkeypatch):
    from app.routers import intimacoes
    ...
    assert "data_publicacao" in str(exc.value.detail)
    assert [o for o in db.added if isinstance(o, Deadline)] == []


@pytest.mark.anyio
async def test_sem_confirmacao_da_fonte_nao_cria_deadline(monkeypatch):
    from app.routers import intimacoes
    ...
    with pytest.raises(HTTPException) as exc:
        await intimacoes.aceitar_prazo(
            com_id="com-1",
            payload=_payload(confirmacao_fonte_oficial=False),
            db=db,
            cu=_user(),
        )
    assert exc.value.status_code == 422
...
@pytest.mark.anyio
async def test_dias_e_rejeitado_mesmo_com_vencimento_informado(monkeypatch):
    from app.routers import intimacoes
    ...
    with pytest.raises(HTTPException) as exc:
        await intimacoes.aceitar_prazo(
            com_id="com-1", payload=_payload(dias=10), db=db, cu=_user()
        )
    assert exc.value.status_code == 422
    assert "não calcula" in str(exc.value.detail)
    assert [o for o in db.added if isinstance(o, Deadline)] == []
...
@pytest.mark.parametrize(
    "payload",
    [
        _payload(data_publicacao=date(2026, 3, 1)),
        _payload(termo_inicial=date(2026, 3, 2)),
        _payload(data_prazo=date(2026, 3, 3)),
    ],
)
async def test_cronologia_invalida_e_rejeitada(monkeypatch, payload):
    from app.routers import intimacoes
...
@pytest.mark.anyio
async def test_quatro_marcos_revisados_criam_deadline_com_snapshot(monkeypatch):
    from app.routers import intimacoes
    ...
    assert prazo.data_publicacao == date(2026, 3, 3)
    assert prazo.termo_inicial == date(2026, 3, 4)
    assert prazo.regime_calculo == "civel"
    assert prazo.data_prazo == date(2026, 4, 10)
    assert prazo.data_intimacao is None
    assert prazo.calculo_metadata["data_disponibilizacao"] == "2026-03-02"
    assert prazo.calculo_metadata["data_publicacao"] == "2026-03-03"
    assert prazo.calculo_metadata["termo_inicial"] == "2026-03-04"
    assert prazo.calculo_metadata["regime_calculo"] == "civel"
    assert comunicacao.data_publicacao == prazo.data_publicacao
    assert comunicacao.termo_inicial == prazo.termo_inicial
    assert comunicacao.regime_calculo == prazo.regime_calculo
...
@pytest.mark.anyio
async def test_aceitar_e_idempotente_antes_de_revalidar_payload(monkeypatch):
    from app.routers import intimacoes

    _liberar_caso(monkeypatch)
    comunicacao = _com(prazo_sugerido_status="aceito", prazo_deadline_id="dl-1")
    existente = Deadline(
        id="dl-1",
...
async def test_recusar_depois_de_prazo_aceito_e_bloqueado(monkeypatch):
    ...
    _liberar_caso(monkeypatch)
    db = _FakeDB([_com(prazo_sugerido_status="aceito", prazo_deadline_id="dl-1")])
    with pytest.raises(HTTPException) as exc:
        await intimacoes.recusar_prazo(com_id="com-1", db=db, cu=_user())
...
async def test_nao_pode_marcar_tratada_sem_decisao_de_prazo(monkeypatch):
    ...
    db = _FakeDB([comunicacao])
    with pytest.raises(HTTPException) as exc:
        await intimacoes.processar(com_id="com-1", db=db, cu=_user())
    assert exc.value.status_code == 422
...
async def test_tratada_so_depois_de_decisao_e_com_auditoria(monkeypatch, decisao):
    ...
    db = _FakeDB([comunicacao])
    out = await intimacoes.processar(com_id="com-1", db=db, cu=_user())
...
async def test_processar_e_idempotente(monkeypatch):
    ...
    _liberar_caso(monkeypatch)
    comunicacao = _com(prazo_sugerido_status="recusado", processada=True)
    db = _FakeDB([comunicacao])
    out = await intimacoes.processar(com_id="com-1", db=db, cu=_user())
