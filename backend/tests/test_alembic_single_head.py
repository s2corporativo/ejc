...
HEAD_REVISION = "143_auditoria_operacional_861"
...
def test_worm_encadeia_apos_skills_uso():
    revision = _script_directory().get_revision("131_audit_logs_worm")
    assert revision.down_revision == "130_ejc_skills_uso"


def test_mni_encadeia_apos_worm():
    revision = _script_directory().get_revision("132_processo_eletronico_mni")
    assert revision.down_revision == "131_audit_logs_worm"


def test_consolidacao_fontes_encadeia_apos_mni():
    revision = _script_directory().get_revision("138_consolida_fontes_ingestao")
    assert revision.down_revision == "132_processo_eletronico_mni"


def test_auditoria_861_encadeia_no_head_da_main_de_origem():
