...
def test_cadeia_ignora_preferido_inelegivel(monkeypatch):
    ...
    assert all(p not in {"anthropic", "maritaca", "groq"} for p, _ in cadeia)
...
async def test_chat_roteamento_off_usa_maritaca_no_merito(monkeypatch):
    """Mesmo com roteamento inteligente desligado, mérito jurídico começa na
    Maritaca; Claude não participa automaticamente."""
    _prep(monkeypatch, ROTEAMENTO_INTELIGENTE_ENABLED=False)
    cap = {}
    _mock_provedor(monkeypatch, cap)
...
async def test_claude_so_entra_quando_requisitado(monkeypatch):
    _prep(monkeypatch, ROTEAMENTO_INTELIGENTE_ENABLED=False)
    cadeia_auto = g._resolver_cadeia("estrategia", None, None)
    assert all(p != "anthropic" for p, _ in cadeia_auto)

    cap = {}
    _mock_provedor(monkeypatch, cap)
    resp = await g.chat(
        [{"role": "user", "content": "análise estratégica"}],
        task_type="estrategia",
        provider_override="anthropic",
    )
