...
class _CauseCpf(Exception):
    constraint_name = "ux_users_cpf_hash_active"


class _OrigComCause(Exception):
    pass
...
def test_integrity_cpf_reconhece_constraint_na_causa():
    orig = _OrigComCause()
    orig.__cause__ = _CauseCpf()
    assert _integrity_e_duplicidade_cpf_usuario(_integrity(orig)) is True
