...
def test_alembic_cadeia_integra():
    ...
    # Head único = 055 (reaplicação da Fase 3B sobre a base de produção, que vai
    # até 054_victory_vault). Cadeia única, sem heads divergentes.
    assert script.get_heads() == ["055_rag_isolation"]
    # walk_revisions percorre head→base; lança se houver down_revision ausente.
    revs = [r.revision for r in script.walk_revisions()]
    assert revs[0] == "055_rag_isolation"          # head é o primeiro no walk
