"""Regressões da regra federal do PAF após LC 227/2026 (Issue #1553)."""
...
def test_transicao_ate_31_marco_escolhe_o_vencimento_posterior() -> None:
    out = calcular_prazo_impugnacao_paf(date(2026, 3, 31))
    assert out["criterio"] == "transicao_adi_rfb_2_2026_maior_vencimento"
    assert set(out["componentes"]) == {"20_dias_uteis", "30_dias_corridos"}
...
def test_apos_transicao_aplica_20_dias_uteis() -> None:
    ...
    assert set(out["componentes"]) == {"20_dias_uteis"}


@pytest.mark.asyncio
async def test_endpoint_federal_explica_regra_e_permanece_nao_homologado() -> None:
    ...
    assert out["reducoes_multa_de_oficio"]["pagamento_ou_compensacao_em_30_dias"]["reducao_pct"] == 50
    assert out["fontes"]
...
def test_agregador_expoe_uma_unica_rota_canônica() -> None:
    rotas = [
        rota
