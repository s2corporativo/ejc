...
class DataRoom(Base):
    ...
    # Alinhado ao limite histórico do v4 (migration 110 amplia de 200 para 255).
    ...
    # Compatibilidade transitória do antigo `dataroom_salas`. Estes campos NÃO
    # tornam a sala pública: acesso externo continua exigindo DataRoomLink ativo,
    # auditável e revogável. São preservados para que a URL v4 mantenha o contrato
    # até sua retirada controlada.
    legacy_expira_em = Column(DateTime(timezone=True), nullable=True)
    legacy_publica = Column(Boolean, nullable=False, default=False, server_default="false")
...
class DataRoomArquivo(Base):
    ...
    id = Column(String(36), primary_key=True)
    data_room_id = Column(
        String(36), ForeignKey("data_rooms.id", ondelete="CASCADE"), nullable=False
    )
    document_id = Column(
...
class DataRoomAcessoLog(Base):
    ...
    id = Column(String(36), primary_key=True)
    link_id = Column(
        String(36), ForeignKey("data_room_links.id", ondelete="CASCADE"), nullable=False
    )
    ip = Column(String(45))
