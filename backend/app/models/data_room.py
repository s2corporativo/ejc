...
class DataRoomArquivo(Base):
    ...
    nome_exibicao = Column(String(300))
    adicionado_por = Column(
        String(36), ForeignKey("users.id", ondelete="SET NULL"), nullable=True
    )
    added_at = Column(DateTime(timezone=True), server_default=func.now())

    # Estar na sala não significa estar publicado externamente. A publicação é
    # uma decisão separada, explícita e auditável. Arquivos legados nascem
    # fechados após a migration 124.
    publicado_externamente = Column(
        Boolean, nullable=False, default=False, server_default="false"
    )
    publicado_por = Column(
        String(36), ForeignKey("users.id", ondelete="SET NULL"), nullable=True
    )
...
class DataRoomLink(Base):
    ...
    # Somente SHA-256 em repouso. O token claro é devolvido uma única vez na
    # criação e nunca reaparece em listagens, banco ou logs.
    token_hash = Column(String(64), unique=True, nullable=False)
    descricao = Column(String(200))
    expira_em = Column(DateTime(timezone=True))
    max_acessos = Column(Integer)
...
class DataRoomAcessoLog(Base):
    ...
    link_id = Column(
        String(36),
        ForeignKey("data_room_links.id", ondelete="CASCADE"),
        nullable=False,
    )
    ip = Column(String(45))
