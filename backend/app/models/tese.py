...
class Tese(Base):
    ...
    tipo = Column(
        SAEnum(TeseTipo), nullable=False, default=TeseTipo.escritorio
    )
    status = Column(
        SAEnum(TeseStatus), nullable=False, default=TeseStatus.ativa
    )

    # Métricas de desempenho comprovadas pelo uso no EJC.
    ...
    # O v4 possuía apenas um booleano `vencedora`, sem quantidade de usos. A
    # migration 110 preserva esse fato histórico sem inventar contagens. A rota
    # canônica continua usando as métricas comprovadas acima; só o adaptador v4
    # interpreta este campo durante a janela de compatibilidade.
    legacy_vencedora = Column(Boolean, nullable=True)
...
class TeseCasoLink(Base):
    ...
    id = Column(String(36), primary_key=True)
    tese_id = Column(
        String(36), ForeignKey("teses.id", ondelete="CASCADE"), nullable=False
    )
    case_id = Column(
