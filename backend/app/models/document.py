...
class Document(Base):
    ...
    `publicado_portal` é deliberadamente independente de `confidencialidade`:
    classificação interna não constitui autorização para publicação ao cliente.
    """
    ...
    )  # procuracao|contrato|decisao|peticao|prova|outro
    ...
    confidencialidade = Column(
        SAEnum(DocConfidencialidade),
        nullable=False,
        default=DocConfidencialidade.confidencial,
        index=True,
    )

    # Publicação externa explícita. Default fail-closed: nenhum documento
    # interno aparece no Portal somente por ser classificado como "normal".
    publicado_portal = Column(
