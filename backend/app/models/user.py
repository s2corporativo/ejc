...
class User(Base):
    ...
    # Versão da credencial pública do feed ICS. Incrementar revoga somente os
    # links antigos daquele usuário, sem rotacionar SECRET_KEY nem afetar colegas.
    calendar_token_version = Column(Integer, default=1, server_default="1", nullable=False)
    ...
    cases_responsible = relationship(
        "Case",
        foreign_keys="Case.advogado_responsavel_id",
        back_populates="advogado_responsavel",
    )
    deadlines = relationship(
        "Deadline", foreign_keys="Deadline.responsavel_id", back_populates="responsavel"
    )
    audit_logs = relationship("AuditLog", back_populates="user")
    ai_logs = relationship("AILog", foreign_keys="AILog.user_id", back_populates="user")
    legal_docs_revisados = relationship(
        "LegalDoc", foreign_keys="LegalDoc.revisor_id", back_populates="revisor"
    )
