    ...
    try:
        ...
        try:
            ...
            if isinstance(citacoes, dict):
                ...
                resultado.citacoes_nao_confirmadas = int(
                    citacoes.get("nao_encontradas") or 0
                )
        except Exception:
            pass
...
def agregar(resultados: list[ResultadoProvider]) -> dict[str, dict[str, Any]]:
    ...
    for provider, itens in por_provider.items():
        ...
        grounded = [
            item.groundedness for item in validos if item.groundedness is not None
        ]
        agregado[provider] = {
            "n_validos": len(validos),
            "fallbacks_excluidos": len(fallbacks),
            "erros": len(erros),
            "taxa_citacoes_nao_confirmadas": (
                round(cit_ruins / cit_total, 4) if cit_total else None
            ),
            "groundedness_media": (
                round(sum(grounded) / len(grounded), 4) if grounded else None
            ),
            "custo_total_brl": round(
                sum(item.custo_brl for item in validos), 6
            ),
            "duracao_media_ms": (
    ...
    async with AsyncSessionLocal() as db:
        for caso in casos:
            ...
            for provider in providers:
                ...
                estado = (
                    f"FALLBACK→{item.provider_real}" if item.fallback
                    else item.erro or "OK"
                )
                print(
                    f"[{caso['id']}] {provider}: {estado}; "
                    f"citações={item.citacoes_nao_confirmadas}/{item.citacoes_total}; "
                    f"ground={item.groundedness}; custo=R${item.custo_brl}; "
                    f"{item.duracao_ms}ms"
                )
