...
# Comparar provedores lado a lado (ex.: Claude x Sabiá) sobre o MESMO contexto
python -m app.eval.run_eval --gold app/eval/gold_set.jsonl --k 6 --full --judge \
...
## Fluxo recomendado (eval-driven — roteiro da auditoria 2026-07-17)

1. **Baseline** — monte 15–20 casos, meça o pipeline atual. Nada muda antes disso.
2. **Compare provedores** — `--providers anthropic,maritaca`: se o Sabiá empatar em
   alucinação/groundedness a custo menor, ele ganha mais tarefas na cadeia; se ficar atrás,
   permanece como fallback/crítico diverso.
3. **Uma variável por vez** — reranker, FTS, embedding: ligue, meça contra o baseline, decida
   pelo número.
4. **Cresça o gold set** — mais casos = régua mais confiável. É trabalho incremental do
   escritório, e o de maior retorno de todos.
