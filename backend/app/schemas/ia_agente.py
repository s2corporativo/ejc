...
class AgenteStreamRequest(BaseModel):
    ...
    # ── HITL vinculado aos ARGS (achado H1 + auditoria 2026-07-26 AI-030) ────
    # Ao pausar numa write-tool, o servidor devolve {token, args_hash}. O ÚNICO
    # caminho de aprovação é retomar_token + decisao (estado servidor-side,
    # vinculado a usuário/caso/papel). O antigo campo `aprovacoes_hash` foi
    # REMOVIDO: hash de dados públicos calculável pelo cliente prova
    # integridade, não autorização (AI-030) — se enviado, é ignorado.
