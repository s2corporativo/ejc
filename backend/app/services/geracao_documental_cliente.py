"""Geração segura de documentos de admissão vinculados ao cliente.

Contrato e procuração nascem sempre como rascunho, sem LLM, com revisão humana
obrigatória. Peças avulsas usam ``LegalDoc.client_id`` como vínculo de domínio;
nome/título nunca é usado como identidade do cliente.
"""
...
AVISO_RASCUNHO = (
    ...
    "Status: RASCUNHO — revisão e aprovação do advogado são obrigatórias."
)
AVISO_KIT_EXISTENTE = (
    "Documentos já existentes para este cliente foram reaproveitados. "
    "Para gerar nova versão, envie forcar_novo=true."
)
AVISO_SEM_AREA_OAB = (
    "Valor a definir: nenhum item vigente da Tabela OAB/MG foi localizado "
    "para a área informada. Consulte a fonte oficial antes da aprovação."
)


def _titulos_canonicos(nome_cliente: str) -> dict[str, str]:
    return {
...
async def listar_pecas_cliente(db: AsyncSession, cli: Client) -> list[dict]:
    """Lista somente peças avulsas pertencentes ao ``client_id`` solicitado."""
    titulos = list(_titulos_canonicos(cli.razao_social or cli.nome or "Cliente").values())
    docs = (
        await db.execute(
            select(LegalDoc)
            .where(
...
async def gerar_documentos_cliente(
    ...
    *,
    ...
    """Gera procuração + contrato vinculados inequivocamente a ``cli.id``."""
    ...
    titulos = _titulos_canonicos(nome_cliente)
    if not forcar_novo:
        existentes = (
            await db.execute(
                select(LegalDoc)
        ...
        for doc in existentes:
            por_titulo.setdefault(doc.titulo, doc)
        if all(titulo in por_titulo for titulo in titulos.values()):
            proc = por_titulo[titulos["procuracao"]]
            contrato = por_titulo[titulos["contrato"]]
            return {
                "client_id": cli.id,
    ...
    for titulo, tipo_peca, conteudo in conteudos:
        doc = LegalDoc(
            id=str(uuid4()),
            titulo=titulo,
            tipo_peca=tipo_peca,
