...
async def auditar_versionamento_documental(
    ...
    # Raiz canônica de um grupo declarado é inválida quando a versão raiz não é
    # a 1, quando o predecessor existe ou quando o doc raiz pertence a outro
    # grupo. Raízes de grupo implícito (documento sem grupo que declara a si
    # mesmo como raiz) são validadas por versão e predecessor apenas — um
    # versao_grupo_id NULL de grupo implícito não é anomalia. O GROUP BY evita
    # contagem duplicada quando mais de um documento aponta para a mesma raiz.
    raizes_invalidas_sub = (
        select(raiz.id.label("raiz_id"))
        .where(raiz.id == _grupo_canonico(raiz))
        .where(
            or_(
                raiz.versao != text("1"),
    ...
    raizes_canonicas_invalidas = (
        await db.scalar(select(func.count()).select_from(raizes_invalidas_sub)) or 0
    )
