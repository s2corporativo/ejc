...
# Sigla de código → slug do ingestor oficial (planalto.CATALOGO).
_SLUG_POR_DIPLOMA = {
    "cf": "cf88",
    "cpc": "cpc",
    "cc": "cc",
...
def _fonte_da_linha(row) -> dict | None:
    if not row:
        return None
    return {
        "doc_id": row[0],
...
async def _fonte_artigo(
    ...
    *,
    ...
    fonte = _fonte_da_linha(row)
    if fonte and fonte["chave_origem"] != params.get("chave"):
        # Guarda redundante contra regressão futura no SQL.
        return None
    return fonte
