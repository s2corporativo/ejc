"""Produtividade — indicadores gerenciais sensíveis (sócio+)."""
...
async def produtividade(
    ...
    for a in adv:
        h = float(a["horas"])
        hf = float(a["horas_faturavel"])
        por_advogado.append(
            {
                "id": a["id"],
                "nome": a["nome"],
                "horas": round(h, 1),
                "horas_faturavel": round(hf, 1),
                "lancamentos": int(a["lancamentos"]),
                "casos": int(a["casos"]),
                "pct_faturavel": round(hf / h * 100) if h else 0,
            }
        )
    ...
    por_area = [
        {
            "area": a["area"],
            "horas": round(float(a["horas"]), 1),
            "horas_faturavel": round(float(a["horas_faturavel"]), 1),
            "casos": int(a["casos"]),
        }
        for a in area
    ]
    ...
    trend = [
        {"dia": str(r["dia"]), "horas": round(float(r["horas"]), 1)}
        for r in trend_rows
    ]
    ...
    return {
        ...
        "resumo": {
            ...
            "horas_faturavel": round(total_hf, 1),
...
class ProdutividadeExportEvent(BaseModel):
    periodo: Literal["7d", "30d", "90d", "365d"]
...
async def registrar_exportacao_produtividade(
    ...
    """Registra a saída de relatório gerencial sem armazenar seu conteúdo."""
    if body.linhas < 0 or body.linhas > 100_000:
        raise HTTPException(status_code=422, detail="Quantidade de linhas inválida")
    await criar_audit_log(
        ...
        detalhes="Exportação de indicador gerencial individual consolidado.",
    )
    await db.commit()
    return {"ok": True}
