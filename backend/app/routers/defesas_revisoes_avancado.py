...
def _decimal(value: Any, default: Decimal = Decimal("0")) -> Decimal:
    ...
    try:
        return Decimal(
            str(value).replace(".", "").replace(",", ".") if isinstance(value, str) and "," in value else str(value)
        )
...
async def critica_adversarial(
    ...
    return await _executar_ia(
        db,
        cu,
        mensagem=mensagem,
        domain=f"defesas_adversarial:{modalidade}",
...
async def calcular_especialidade(body: dict, cu: User = Depends(get_current_user)):
    ...
    if modalidade in {"multa_ambiental", "multa_administrativa"}:
        ...
        return {
            ...
            "memoria": [
                f"Base: {valor_base}",
                f"Agravantes: {agravantes}%",
                f"Reincidência: {reincidencia}%",
...
async def persistir_resultado(
    ...
    if checklist_itens:
        ...
        for ordem, item in enumerate(checklist_itens, 1):
            ...
            db.add(
                CaseChecklistItem(
                    id=str(uuid4()),
                    case_checklist_id=checklist.id,
                    texto=texto[:500],
                    dica=dica[:4000] or None,
                    categoria=ChecklistItemCategoria.documentos,
                    obrigatorio=impeditivo,
                    ordem=ordem,
                    concluido=atendido,
    ...
    for documento in _lista(resultado.get("documentos_faltantes"))[:20]:
        tarefa = Task(
            id=str(uuid4()),
            titulo=f"Obter: {str(documento)[:220]}",
            descricao="Pendência identificada pela jornada Defesas e Revisões.",
            status=TaskStatus.a_fazer,
            prioridade="alta" if completude["nivel"] == "nao_apto_para_redacao" else "media",
            case_id=case_id,
            responsavel_id=caso.advogado_responsavel_id or cu.id,
            criado_por=cu.id,
        )
        db.add(tarefa)
    ...
    db.add(
        Task(
            id=str(uuid4()),
            titulo="Acompanhar decisão e importar novo ato",
    ...
    return {
        ...
        "completude": completude,
        "matriz_teses": matriz,
        "tarefas_documentais": tarefas,
        "status": "rascunho",
        "revisao_obrigatoria": True,
    }


# B3/W5 (consolidação): a rota POST /pacote vive NESTE arquivo — a implementação
# segura (bloqueia kit/motor quando há pendência impeditiva) foi fundida do
# antigo defesas_revisoes_pacote_seguro.py, encerrando o duplo mount do prefixo
# /defesas-revisoes/avancado. A implementação legada, que gerava procuração e
# honorários sem gate de completude, permanece REMOVIDA — não reintroduzir.
        ...
        itens.append(
            {
                ...
                "resultado_final": caso.resultado,
                "motivo_resultado": caso.motivo_resultado,
...
# ── Pacote jurídico seguro (fusão B3/W5) ─────────────────────────────────
...
def _texto_caderno_provas(resultado: dict[str, Any]) -> str:
    linhas = [
        "# Caderno de provas",
        "",
        "Relação estruturada; a autenticidade e pertinência devem ser confirmadas.",
        "",
    ]
    provas: list[str] = []
    for item in _matriz_teses(resultado):
