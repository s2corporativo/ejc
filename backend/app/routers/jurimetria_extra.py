...
async def analise_prospectiva(
    ...
    A taxa favorável usa apenas desfechos DECIDIDOS. Para preservar os contratos
    históricos de encerramento de caso, `exito`, `exito_total` e `exito_parcial`
    são favoráveis; `derrota` e `improcedente` são desfavoráveis. Acordos são
    mostrados separadamente e não são tratados como vitória judicial. Outros
    resultados não classificáveis ficam fora do denominador.
    """
    ...
    favoraveis = (
        contagens.get("exito", 0)
        + contagens.get("exito_total", 0)
        + contagens.get("exito_parcial", 0)
    )
    desfavoraveis = contagens.get("derrota", 0) + contagens.get("improcedente", 0)
