...
| 139_ai_log_fk_ondelete_set_null | 138_consolida_fontes_ingestao | stabilization/ejc-2026-08 | (a abrir) | ChatGPT | Reservada | Hardening da Sala Jurídica: a expiração/purga de `ai_logs` não deve falhar por FK de mensagens históricas; preserva a mensagem e zera apenas o vínculo ao log expirado via `ON DELETE SET NULL`. PRs experimentais 140–142 que ainda partem de 138 precisam ser rebaseados/renumerados após a integração desta revisão. |
