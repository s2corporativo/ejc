...
| 130_ejc_skills_uso | 127_publicacao_explicita | claude/ejc-skills-uso-tracking | — | Claude Code | Mesclada | Bloco 4 (enxugar catálogo): contador `vezes_executado`/`ultima_execucao` em `ejc_skills`. Status corrigido nesta linha em 2026-08-07 (tabela estava desatualizada — o arquivo já está na `main`; ver `backend/alembic/versions/130_ejc_skills_uso.py`). |
| 131_audit_logs_worm | 130_ejc_skills_uso | claude/audit-worm-699 | — | Claude Code | Mesclada | Issue #699: impõe WORM em `audit_logs` via trigger `BEFORE UPDATE OR DELETE`. Status corrigido nesta linha em 2026-08-07 (tabela estava desatualizada — o arquivo já está na `main`, é o head atual confirmado por `down_revision` órfão). **Head da `main`** até a reserva abaixo. |
| 132_ai_skills_remover_engine_ollama | 131_audit_logs_worm | claude/relatorio-modulos-analise-fla9gd | (a abrir) | Claude Code | Reservada | Remove `'ollama'` do CHECK constraint `engine IN (...)` de `ejc_skills` (criado em `051_ejc_skills.py`) e reatribui as 3 skills seedadas com `engine='ollama'` para `engine='groq'`, como parte da remoção completa do provider Ollama do sistema (pedido do titular em chat, Issue a abrir). |
