...
| 132_processo_eletronico_mni | 131_audit_logs_worm | claude/execute-implemente-mo6j4v | [#763](https://github.com/s2corporativo/ejc/pull/763) | Claude Code | Mesclada | Fase A MNI/TJMG: `credencial_processo_eletronico`, `tribunal`, tabelas de sincronização. **Head da `main`.** Corrigida em [#784](https://github.com/s2corporativo/ejc/pull/784) — ver linha abaixo. |
| 132_processo_eletronico_mni (correção do seed) | 131_audit_logs_worm | claude/migration-132-backfill-gate | [#784](https://github.com/s2corporativo/ejc/pull/784) | Claude Code | Em PR | **Não é número novo** — corrige a 132 acima, que ainda **não foi aplicada em produção** (que segue em `131_audit_logs_worm`), portanto a regra "migration aplicada não se edita" não é violada. O seed do catálogo usava `op.bulk_insert` sobre `sa.table()`, que o `scripts/check_migration_compatibility.py` reprova; o deploy de 2026-08-07 12:08 UTC saiu com exit 1 na etapa "Classificar migrations pendentes" e todo push seguinte na `main` repetia a falha. Passa a `op.execute` com `INSERT ... SELECT ... WHERE NOT EXISTS` sob `deployment_policy = "additive_data_backfill"`, e os UUIDs do seed viram fixos (com `uuid4()` a mesma linha teria identidade diferente por ambiente). Issue [#783](https://github.com/s2corporativo/ejc/issues/783). |
