...
| 148_banco_teses_juridicas | 147_pendencia_impacto_providencia | claude/banco-teses-schema-validacao | Issue [#1266](https://github.com/s2corporativo/ejc/issues/1266) (PR a abrir) | Claude Code | Reservada | **Reescrita (Fase 2 da série).** O arquivo original (PR #1264/Fase 1) criava tabelas paralelas (`taxonomias`/`teses_juridicas`/`precedentes`/`processos_vitoriosos`/`contrateses_tese`) que duplicavam o Banco de Teses canônico (`teses`/`tese_caso_links`, migração 014) — removido nessa fase (ver `docs/decisoes/ADR_BANCO_TESES_CANONICO_2026-08-24.md`). Reaberta aqui como consolidação ADITIVA da tabela canônica `teses`/`jurisprudencias_internas` (padrão da migração 114): colunas novas + tabelas satélite `teses_fundamentacoes`/`teses_relacoes`/`teses_jurisprudencias`/`teses_alertas_jurisprudenciais`/`legal_evidence`. |
| 149_tese_embedding_radar | 148_banco_teses_juridicas | claude/banco-teses-radar-jurisprudencial | (a abrir) | Claude Code | Reservada | **Fase 4 da série — Radar Jurisprudencial, Camada 2 (semântica).** Coluna `teses.embedding vector(1024)` + índice HNSW cosine, mesmo padrão de `096_rag_embedding_1024.py`. Nullable, sem backfill: teses existentes ficam sem embedding até serem (re)computadas por `radar_jurisprudencial_embedding.py`. Empilhada sobre a 148 (ainda não mesclada) — mesma lógica de PR empilhado já usada pela série (147→148). |
