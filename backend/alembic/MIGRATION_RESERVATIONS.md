...
| 148_banco_teses_juridicas | 147_pendencia_impacto_providencia | (a abrir — PR 2 da série) | [#1264](https://github.com/s2corporativo/ejc/pull/1264) (Fase 1, decisão) | Claude Code | Reservada | **Reescrita.** O arquivo original criava tabelas paralelas (`taxonomias`/`teses_juridicas`/`precedentes`/`processos_vitoriosos`/`contrateses_tese`) que duplicavam o Banco de Teses canônico (`teses`/`tese_caso_links`, migração 014) — removido do PR #1264 (Fase 1: estabilização, ver `docs/decisoes/ADR_BANCO_TESES_CANONICO_2026-08-24.md`). O número 148 permanece reservado: será reaberto no PR 2 da série como extensão ADITIVA da tabela canônica. Fase 1 não contém nenhuma migração. |
