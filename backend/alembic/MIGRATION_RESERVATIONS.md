...
| 137_documents_sha256 | 131_audit_logs_worm | claude/documents-upload-hardening-697 | (a abrir) | Claude Code | Em PR | Issue #697: coluna `documents.sha256` (nullable, indexada) — hash de integridade calculado no upload do GED, único caminho de ingestão do repositório sem hash (document_intake/raio_x/legal_chat/signature já têm). Sem backfill dos documentos existentes (Issue própria) nem da via `POST /documents/drive/upload` (INSERT bruto). **Numeração 137, não 132 — landscape na data desta reserva:** o PR #746 (mesclado, atualizou esta tabela em 2026-08-06) já reserva `132_case_parte_pii_encriptado` (fatia 652-C, PR #746/#652) e `133–135` (reconstrução do #679, mesmo PR #746); independentemente, a branch `claude/portal-varredura-publicacao-698` (Issue #698, mesmo arquivo `documents.py`, região diferente — publicação no portal) já tem os ARQUIVOS `133_user_password_changed_at`, `134_processes_numero_cnj_index`, `135_indice_risco_nivel_size` e `136_document_publicacao_portal` criados (down_revision=131, não 132 como o #746 planejava — as duas reservas de 133–135 já divergem entre si e não são problema desta migration). 137 é o primeiro número sem reserva nem arquivo conhecido nesta data. `down_revision=131_audit_logs_worm`, mesmo pai de `133_user_password_changed_at` (#698) e de `132_case_parte_pii_encriptado` (#746, ainda não criada) — três cadeias saindo do mesmo head; quem mesclar por último rebasa `down_revision` sobre o head então vigente, como já ocorreu antes com 122/124 (ver linhas revogadas acima). |
