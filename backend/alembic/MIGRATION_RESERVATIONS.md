...
| 139_ai_log_fk_ondelete_set_null | 138_consolida_fontes_ingestao | claude/f1a-casos-sala-estabilizacao | (a abrir) | Claude Code | Em PR | F1a (docs/PLANO_FUSAO_CASO_UNICO.md): `legal_chat_messages.ai_log_id` ganha `ON DELETE SET NULL` — era a única FK do sistema para `ai_logs.id`, sem `ondelete`; a purga LGPD de `_purgar_logs_ia` batia nela e falhava em silêncio assim que existisse mensagem da Sala com log expirado. Nasceu como 133/down=132; o PR #786 mesclou primeiro consumindo 138 do mesmo pai (132) — renumerada para 139 e reencadeada em 138 na integração. |
...
| 138_consolida_fontes_ingestao | 132_processo_eletronico_mni | claude/ejc-refactoring-complete-tifnb5 | [#786](https://github.com/s2corporativo/ejc/pull/786) | Claude Code | Mesclada | Onda 2 — consolidação de fontes de ingestão: mescla as linhas `juris_import_stj|tjmg|lexml` de `fontes_ingestao` nas canônicas `stj|tjmg|lexml` (soma contadores, OR de `ja_produziu`/`ativo`, timestamp mais recente; renomeia quando a canônica não existe). Encadeada na 132 porque a branch da Refatoração Total já contém a 132 como head local; números 133–137 pulados (reservados/tomados por PRs abertos). **Head da `main`** até a 139 acima. |
