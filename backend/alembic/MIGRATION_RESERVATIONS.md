...
| 142_preliminares_fundacao_schema | 138_consolida_fontes_ingestao | claude/f5-fusao-schema-preliminares | [#812](https://github.com/s2corporativo/ejc/pull/812) | Claude Code | Em PR | F5 Fase 1 (Issue #799): fundação de schema da fusão Raio-X + Sala Jurídica — cria `preliminares`/`preliminar_documentos`/`preliminar_mensagens`/`preliminar_estados`, estritamente aditiva (sem tocar `raio_x_*`/`legal_chat_*`, sem migrar dado, sem mudar service/router). Encadeada na 138 (head da `main` no momento da reserva) — se #795 (139), #803 (140) ou #806 (141) mesclarem primeiro, esta migration precisa ser renumerada para 143 e reencadeada na que mesclou por último, mesmo padrão já aplicado quando o PR #786 mesclou antes da 139 original. Fases 2 (backfill/dual-write) e 3 (cutover/aposentadoria das tabelas antigas) ficam para Issues futuras. |
