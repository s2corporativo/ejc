...
| 132_processo_eletronico_mni | 131_audit_logs_worm | claude/execute-implemente-mo6j4v | [#762](https://github.com/s2corporativo/ejc/issues/762) | Claude Code | Reservada | Fase A MNI/TJMG: `credencial_processo_eletronico`, `tribunal`, tabelas de sincronização. |
...
| 138_consolida_fontes_ingestao | 132_processo_eletronico_mni | claude/ejc-refactoring-complete-tifnb5 | (a abrir) | Claude Code | Reservada | Onda 2 — consolidação de fontes de ingestão: mescla as linhas `juris_import_stj|tjmg|lexml` de `fontes_ingestao` nas canônicas `stj|tjmg|lexml` (soma contadores, OR de `ja_produziu`/`ativo`, timestamp mais recente; renomeia quando a canônica não existe). Encadeada na 132 porque a branch da Refatoração Total já contém a 132 como head local; números 133–137 pulados (reservados/tomados por PRs abertos). |
| 139_ai_log_fk_ondelete_set_null | 138_consolida_fontes_ingestao | claude/f1a-casos-sala-estabilizacao | [#795](https://github.com/s2corporativo/ejc/pull/795) | Claude Code | Em PR | F1b: `legal_chat_messages.ai_log_id` ganha `ON DELETE SET NULL`. |
| 140_legal_chat_retention_purga | 138_consolida_fontes_ingestao | claude/f4-retencao-lgpd-preliminares | [#803](https://github.com/s2corporativo/ejc/pull/803) | Claude Code | Em PR | F4: `legal_chat_sessions.retention_until` + job de purga de preliminares abandonadas. |
| 141_case_despesas_processuais | 138_consolida_fontes_ingestao | claude/f3-2-despesas-processuais | (a abrir) | Claude Code | Em PR | F3.2 (Issue #806): tabela `case_despesas` — despesa processual lançada pelo advogado no caso, distinta de `office_expenses` (overhead, RBAC restrito). Encadeada na 138 (head da `main` no momento da reserva) — **se #795 (139) ou #803 (140) mesclarem primeiro, esta migration precisa ser renumerada para 142 e reencadeada na que mesclou por último**, mesmo padrão já aplicado quando o PR #786 mesclou antes da 139 original. |
