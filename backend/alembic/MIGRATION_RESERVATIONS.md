...
| 133_user_password_changed_at | 132_case_parte_pii_encriptado | claude/fixes-without-github-45v7ep | [#679](https://github.com/s2corporativo/ejc/pull/679) | Claude Code | Reservada | Renumerada de 128 em 2026-08-05. Coluna que rastreia quando a senha atual foi definida; permite `get_current_user` rejeitar tokens antigos. Nullable sem backfill para evitar desautorizar sessões ativas no deploy. |
| 134_processes_numero_cnj_index | 133_user_password_changed_at | claude/fixes-without-github-45v7ep | [#679](https://github.com/s2corporativo/ejc/pull/679) | Claude Code | Reservada | Renumerada de 129 em 2026-08-05. Índice de busca por `numero_cnj` — melhora performance de pesquisa (DADOS-003 parcial). Índice não único; constraint UNIQUE adiada até validação em produção. |
