...
| 128_user_password_changed_at | 126_case_status_quatro_estados | claude/fixes-without-github-45v7ep | [#679](https://github.com/s2corporativo/ejc/pull/679) | Claude Code | Reservada | Marca de quando a senha atual foi definida, para `get_current_user` recusar access token anterior à troca. **O número 127 foi deliberadamente pulado**: está em uso pelo PR #652 (`127_case_parte_pii_encriptado`), aberto e ainda não mesclado — reusá-lo colidiria na integração. Coluna nullable, sem backfill: preencher com `now()` derrubaria toda sessão ativa no deploy. |
| 129_processes_numero_cnj_index | 128_user_password_changed_at | claude/fixes-without-github-45v7ep | [#679](https://github.com/s2corporativo/ejc/pull/679) | Claude Code | Reservada | Índice NÃO ÚNICO em `processes.numero_cnj` (DADOS-003, parcial). A parte UNIQUE ficou de fora: exige checar duplicata em produção antes, o que este ambiente não tem acesso para fazer. |
