...
| 139_ai_log_fk_ondelete_set_null | 138_consolida_fontes_ingestao | claude/f1a-casos-sala-estabilizacao | [#795](https://github.com/s2corporativo/ejc/pull/795) | Claude Code | Em PR | F1b: `legal_chat_messages.ai_log_id` ganha `ON DELETE SET NULL` — sem isso, `_purgar_logs_ia` quebra em silêncio ao apagar um `ai_logs` referenciado por mensagem da Sala. |
| 140_legal_chat_retention_purga | 138_consolida_fontes_ingestao | claude/f4-retencao-lgpd-preliminares | (a abrir) | Claude Code | Em PR | F4 (Issue #798): adiciona `legal_chat_sessions.retention_until` (paridade com `RaioXAnalise`) + job de purga de análises/sessões preliminares abandonadas. Encadeada na 138 (head da `main` no momento da reserva) — **se o PR #795 (139) mesclar primeiro, esta migration precisa ser renumerada para 141 e reencadeada em 139**, mesmo padrão já aplicado quando o PR #786 mesclou antes da 139 original. |
