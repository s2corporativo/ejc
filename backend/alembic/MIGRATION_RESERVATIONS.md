...
| 133_user_password_changed_at | 131_audit_logs_worm | claude/fixes-without-github-45v7ep | [#679](https://github.com/s2corporativo/ejc/pull/679) | Claude Code | Em PR | Marca de quando a senha atual foi definida, para `get_current_user` recusar access token anterior à troca. Renumerada de 128 em 2026-08-05. Coluna nullable, sem backfill: preencher com `now()` derrubaria toda sessão ativa no deploy. A proteção passa a valer a partir da próxima troca de cada usuário. **`down_revision` corrigido de `132_case_parte_pii_encriptado` para `131_audit_logs_worm` em 2026-08-06 (item A1)**: 132 é do #652, ainda não mesclado — esta branch nunca teve esse arquivo, então apontar para ele deixava o grafo órfão. A cadeia pula 132 (mesmo padrão que a `main` já usou pulando 128/129). |
...
| 135_indice_risco_nivel_size | 134_processes_numero_cnj_index | claude/fixes-without-github-45v7ep | [#679](https://github.com/s2corporativo/ejc/pull/679) | Claude Code | Em PR | Expande `cases.risco_nivel` e `indice_risco_historico.nivel` de VARCHAR(10) para VARCHAR(15) — o rótulo "indeterminado" (13 caracteres) não cabia e `/recalcular` respondia 500. **`down_revision` corrigido de `132_case_parte_pii_encriptado` para `134_processes_numero_cnj_index` em 2026-08-06 (item A1)**: o arquivo já continha, na docstring, "Revises: 134_processes_numero_cnj_index" — o campo `down_revision` divergia do próprio cabeçalho, resíduo de uma renumeração anterior que não propagou. `alembic.script.ScriptDirectory.get_heads()` confirma head único (`135_indice_risco_nivel_size`) após a correção. |
| 136_document_publicacao_portal | 135_indice_risco_nivel_size | claude/portal-varredura-publicacao-698 | Trilha A, item A3 (`Closes #698`) | Claude Code | Em PR | Publicação explícita de documento no Portal do Cliente — `documents.publicado_portal`/`publicado_por`/`publicado_em`, espelhando o desenho do Data Room (`DataRoomArquivo`, Issue #547). A 127_publicacao_explicita (Fase 0) só mudou o default de `confidencialidade` e reclassificou em massa; não criou o ato de publicação nem o registro de autor/data para o Portal — esta migration fecha essa lacuna (Fase 1). Cadeia partiu de `origin/claude/fixes-without-github-45v7ep` (commit `fc9a127`, head local `135_indice_risco_nivel_size`) por depender do merge do PR #679, que ainda não está na `main`. Só adiciona colunas + roda uma consulta de CONTAGEM diagnóstica (sem UPDATE) — decisão do titular sobre publicar retroativamente os documentos hoje `normal` fica pendente, registrada no corpo do PR. |
