...
| 147_pendencia_impacto_providencia | 146_case_sigilo_reforcado | claude/ejc-strategic-evolution-2qqasi | (a abrir) | Claude Code | Reservada | Issue #1244 (frente 12 de `docs/estrategia/EVOLUCAO_ESTRATEGICA_EJC.md`): colunas `impacto varchar(10)` e `providencia varchar(30)`, ambas NULLABLE, em `client_pending_items`. A lista de pendências registrava o QUE falta e em que estágio está, mas não quanto pesa nem como resolver — os dois campos que convertem inventário em plano de ação. Puramente aditiva, sem backfill; vocabulário fechado no schema Pydantic, como já ocorre com `type`/`status` na mesma tabela. |
| 148_case_status_anterior | 147_pendencia_impacto_providencia | claude/ejc-strategic-evolution-2qqasi | (a abrir) | Claude Code | Reservada | Issue #1272 (Classe B do plano-mestre): coluna `status_anterior varchar(20)` NULLABLE em `cases`. `arquivar`/`encerrar` sempre forçavam `desarquivar`/`reabrir` de volta para `aberto`, descartando o estado de trabalho real (`em_instrucao`/`em_producao`/`protocolado`) que o caso tinha antes de virar terminal. Puramente aditiva, sem backfill — casos já arquivados/encerrados reabrem em `aberto` (comportamento legado preservado) até serem arquivados/encerrados de novo sob o código novo. |
| 149_thesis_candidate_tese_banco | 148_case_status_anterior | claude/ejc-strategic-evolution-2qqasi | (a abrir) | Claude Code | Reservada | Issue #1272 (Classe A do plano-mestre): coluna `tese_banco_id varchar(36)` NULLABLE (FK `teses.id`, `ON DELETE SET NULL`) em `thesis_candidates`. Fecha a ponte ausente entre a matriz de trabalho (`thesis_candidates`) e o Banco de Teses institucional (`teses`+`tese_caso_links`): ao montar a matriz o sistema já sabe de qual `Tese` do banco cada candidata veio (`matriz_teses_service.montar_matriz`), mas descartava essa referência — aprovar uma candidata nunca criava o vínculo em `tese_caso_links`, e por isso `conversao_caso.py` e `legal_case_orchestrator.py` respondiam `tese_aprovada` de forma disjunta para o mesmo caso. Puramente aditiva, sem backfill — não há como reconstruir com confiança de qual `Tese` uma candidata JÁ aprovada antes desta migration se originou (o mapeamento só existia em memória durante `montar_matriz`); candidatas aprovadas a partir de agora materializam o vínculo automaticamente. |
