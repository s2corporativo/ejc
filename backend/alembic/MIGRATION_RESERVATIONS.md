...
| 146_case_sigilo_reforcado | 145_drop_orphan_db_only_columns | main | [#1195](https://github.com/s2corporativo/ejc/pull/1195) | Claude Code | Mesclada | Issue #1194: coluna `sigilo_reforcado boolean not null default false` em `cases` — achado do `security-auditor` provou que o piso `LOCAL_COMPLETO` de crimes sexuais/menores (redução do AI-019 pedida pelo titular) era inalcançável via `Case.area` (sem granularidade) e via `task_type` (nenhuma rota passa texto livre com a palavra certa). Campo explícito, marcado pelo advogado na triagem, consultado com prioridade em `orchestrator.py`/`agent/loop.py`. Puramente aditiva.
