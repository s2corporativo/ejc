...
Use apenas metadados. O SQL é enviado por `stdin`, enquanto usuário e banco são resolvidos **dentro do contêiner `ejc_db`**:

```bash
docker exec -i ejc_db sh -lc 'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB"' <<'SQL'
SELECT
