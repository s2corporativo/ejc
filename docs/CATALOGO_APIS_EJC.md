...
| **DJEN / Comunica CNJ** | Captura de intimações/publicações por OAB; vincula ao caso, cria movimento e alerta (o advogado define o prazo — decisão HITL) | Grátis, sem auth | ✅ `services/djen_service.py` + `services/ingestors/djen.py`. Default `DJEN_INGEST_ENABLED=True`; OABs configuradas: `252599/MG` e `251174/MG`. WAF bloqueia runner GitHub — rodar da VPS | Homologar a primeira captura real e acompanhar `/ia-governanca/fontes` |
