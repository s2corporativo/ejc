...
| **DJEN / Comunica CNJ** | Captura de intimações/publicações por OAB; vincula ao caso, cria movimento e alerta (o advogado define o prazo — decisão HITL) | Grátis, sem auth | **Dois fluxos independentes, com fontes de OAB diferentes.** ✅ **Ingestão RAG** (`services/ingestors/djen.py`, job 05h): lê `DJEN_OABS_MONITORADAS` do env (`252599/MG,251174/MG`) — configurado. 🔧 **Alerta de prazo** (`services/djen_service.py::capturar_para_advogado`, job 06h30 em `scheduler.py`): NÃO lê o env — itera `users` com `djen_oab_numero` preenchido; com o campo vazio o job termina "ok" sem capturar nada. WAF bloqueia runner GitHub — rodar da VPS | Preencher `djen_oab_numero`/`djen_oab_uf` no perfil de CADA advogado (autoatendimento em `PATCH /users/{id}`; teste com `POST /intimacoes/capturar-agora`) e só então homologar a 1ª captura real |
| **PNCP — Contratações Públicas** (Lei 14.133/2021) | Consulta pública de contratações por data/UF/município/modalidade (contexto de direito administrativo) | Grátis, sem chave | ❌ REMOVIDO (2026-07) junto com o módulo de licitações; variáveis `PNCP_*` são ignoradas | Reintroduzir só se houver demanda, com flag default OFF |
...
4. **DJEN + TJMG** — ligar as flags após validar na VPS. (PNCP foi removido em 2026-07.)
