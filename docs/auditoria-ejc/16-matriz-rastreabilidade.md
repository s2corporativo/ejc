...
| **Contrato do escritório** | `OfficeContracts` | `/api/v1/office-contracts` | `office_contracts.py` | `office_contracts` | — | — | **nenhum** | ❌ **QUEBRADO (P0-1)** |
...
| **Gerar peça** | `Pecas`, `PecaGeneratorModal` | `POST /api/pecas/gerar`, `/api/document-templates/generate` | `peca_service`, `motor_peca_service` | `legal_docs` | `LegalWritingAgent` | `generate_legal_draft`, `validate_citations` | `test_motor_peca.py` (orquestrador) | ❌ **`peca_geracao_router` SEM teste** (T-P0-1); `document-templates` **sem gate de advogado** (P1) |
...
| Andamento processual | `TabProcessos`, `DiarioOficial` | `/api/processes`, `/api/v1/datajud` | `datajud_service` | `processes`, `case_movimentos` | — | — | parcial | ❌ **DataJud QUEBRADO (P0-1)** |
...
| **Lançar despesa** | `Despesas` | `/api/v1/despesas` | `despesas.py` | `office_expenses` | — | — | **nenhum** | ❌ **QUEBRADO (P0-1)** |
| Encerrar caso | `Casos` | `DELETE /api/cases/{id}` | `cases.py:613-691` | `cases.deleted_at` | — | — | `test_casos_dblevel.py` | ✅ soft + 422 protetivo |
| **Alimentar base de conhecimento** | `KnowledgeGovernancePanel` | `GET /api/rag/docs`, `/api/rag/governanca` | `ingestion_service`, `knowledge_governance` | `knowledge_docs`, `knowledge_chunks` | — | `modulo_conhecimento` | `test_rag_gate_governanca_dblevel.py` | ❌ **`GET /rag/docs` 500 SEMPRE (P0-3)** |
...
| Cifrar CPF/CNPJ em repouso | — | — | `pii_crypto` (Fernet + HMAC) | `clients.cpf_enc/hash` | `test_client_pii_cutover_dblevel.py` | ❌ **`case_partes.cpf_cnpj` em texto puro (P1)** |
...
| Direito ao esquecimento (art. 17) | `Clientes` | `POST /api/clients/{id}/anonimizar` | `client_anonimizacao` | `clients` | `test_client_anonimizacao_dblevel.py` | ❌ **não cobre `case_partes`, `users`, campos livres (P1)** |
| Trilha de auditoria imutável | `Auditoria` | `/api/audit` | `criar_audit_log` | `audit_logs` | parcial | ❌ **sem WORM — apagável (P2)** |
| Limitar custo de IA | — | `/api/ai/*` | `ai_gateway` | `ai_logs` | — | ❌ **14 rotas POST, 3 com rate limit; sem `default_limits` (P1)** |
| Kill-switch de IA | `GovernancaIA` | — | `ai_gateway` | — | — | ❌ **`AI_ENABLED` não aplicado no gateway (P1)** |
...
| **Tabela sem cobertura de anonimização** | `case_partes`, `sociedades_cliente`, `users` | P1-5 |
| **Verificador cego** | `api_contract.py` não modela o interceptor | P0-2 |

## 4. Como usar esta matriz

1. **Nenhuma linha com ❌ pode ser declarada concluída.** São 12 linhas.
