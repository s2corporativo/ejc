...
**Frente funcional — captura DJEN (item 0.2, CRÍTICO) — verificada como já resolvida em 2026-08-24, sem tocar código:**
- Diagnóstico completo: `services/djen_service.py::capturar_para_advogado` já captura POR advogado (não uma única conta administrativa); `scheduler.py::job_djen_intimacoes` já itera todo `User` ativo com `djen_oab_numero` preenchido. Monitoramento de **resultado**, não execução, já está generalizado (não só DOU): `services/ingestao_saude.py` (`nunca_produziu`/`parou_de_produzir`, PR #1015) alimenta `heartbeat_service.py::_normalizar_resultado_djen`, exposto em `GET /ia-governanca/fontes` e no painel Governança de IA (aba "Fontes", que já cita a própria armadilha do CLAUDE.md no comentário do componente). Autoatendimento já existe: cada advogado configura a própria OAB em "Minha OAB (intimações DJEN)" no menu do avatar (`SecurityMenu.tsx`), com nudge explícito na tela de Intimações vazia. 59 testes (`test_ingestao_diagnostico_fontes.py`, `test_ingestao_saude.py`, `test_heartbeat_resultado.py`) passam contra Postgres real.
- **O que de fato falta não é código — é dado.** "Nunca capturou nada" era verdade em julho/2026 porque nenhum dos 3 advogados reais tinha OAB cadastrada (só a conta administrativa) — e continua sendo verdade em produção até alguém preencher o campo. Isso é exatamente o Bloco 6 (`V3-B6`, cadastro pré-operação) e pertence à trilha do titular (T4), não a esta fase.
...
- Item 2.1 (CRÍTICO) — **verificado como já resolvido em 2026-08-24**, sem tocar código: o achado da auditoria ("`validar` não grava `validacao_juridica.ai_log_id`") já não procede. `services/validador_juridico_service.py::validar_rascunho_juridico` grava `legal_doc_id`/`legal_doc_content_hash`/`legal_doc_validation_current` com hash pinado e lock de linha; `routers/legal_docs.py::validar_peca_juridica` já monta o marcador `LEGAL_DOC_ID:{d.id}` (migration `123_legal_doc_ai_log_vinculo`, PR #1015, 2026-08-14). 47 testes (`test_legal_doc_ai_log_vinculo_dblevel.py`, `test_legal_doc_flow_contract.py`, `test_peca_conferir_assinar.py`) passam contra Postgres real com as migrations até `147`. Item 2.4 (consolidar aprovação em um único ato) também já resolvido — `POST /{doc_id}/conferir-e-assinar` (PR #1153, 2026-08-18).
  - **Correção da hipótese original deste plano sobre a Classe D:** a suposição de que `fee_proposal_service.py` "cria proposta já `status=aprovada` sem ato humano" estava **errada** — o mesmo vício de ausência falsa que este documento adverte em outro lugar. `seed_proposta_honorarios_cadastro` exige `_req_advogado_service` (RBAC real sobre o usuário que está criando o caso) e atribui `aprovado_por` a esse mesmo usuário; um papel abaixo de advogado recebe 403 e o caso não é criado. Já há teste dedicado (`test_seed_honorarios_exige_advogado_bloqueia_papel_inferior`, `test_fee_proposal.py`) provando o bloqueio. Não há correção a fazer aqui — a "aprovação" é o próprio ato do advogado ao preencher os honorários na abertura, e o RBAC já impede escalonamento de privilégio.
  - O que resta da Classe D (nascimento automático de 7/16 etapas da Jornada) é só `CL-D1`/`CL-D2` — "Documentos lidos" e a ponte `case_checklists` → `checklist_criado` — permanece escopo da F2, não desta fase.
