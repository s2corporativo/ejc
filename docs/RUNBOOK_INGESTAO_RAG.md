...
| `DJEN_INGEST_ENABLED` | `true` | etapa DJEN → RAG dentro do job canônico `djen` das 06h30. A captura dos advogados cadastrados continua no mesmo ciclo; OABs extras de `DJEN_OABS_MONITORADAS` só são consultadas se ainda não tiverem sido coletadas naquele ciclo |
...
| `djen` | diário 06h30 | Captura DJEN dos advogados + RAG de OABs adicionais, com deduplicação por OAB/ciclo |
...
| DJEN (intimações) | job canônico `djen`: advogados cadastrados + OABs adicionais configuradas, sem segunda consulta da mesma OAB no ciclo |
